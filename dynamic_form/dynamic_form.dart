...
class DynamicFormState extends State<DynamicForm> {
  ...
  /// - Grid fields (all rows): Pass fieldKey only (affects field definition)
  /// - Grid fields (specific row): Pass fieldKey with index parameter
  ///
  /// Parameters:
  /// - [fieldKey]: The key of the field to show/hide (without @index for grid fields)
  /// - [isVisible]: true to show the field, false to hide it
  /// - [index]: Optional row index for grid fields
  ///
  /// Examples:
  /// ```dart
  /// // Normal field
  ...
  ///
  /// // Grid field (specific row) - Note: This affects the field definition, not individual cells
  /// // For row-specific visibility, use field-level logic in the grid component
  /// dynamicFormKey.currentState?.setFieldVisibility('customerName', false, index: 0);
  /// ```
  void setFieldVisibility(String fieldKey, bool isVisible, {int? index}) {
    // For grid fields with index, construct the flattened key
    // Note: This still updates the field definition, not individual cells
    // Grid field visibility is typically controlled at the field level, not per-cell
    final searchKey = index != null ? '$fieldKey@$index' : fieldKey;

    // Find and update the field's isActive property
    for (Section section in widget.sections) {
      for (RowElement row in section.rows ?? []) {
        for (DynamicField field in row.fields ?? []) {
          // Match either the base key or the flattened key
          if (field.key == fieldKey || field.key == searchKey) {
  ...
  ///
  /// // Grid field (specific row) - Note: This affects the field definition, not individual cells
  /// // For row-specific mandatory status, use field-level logic in the grid component
  /// dynamicFormKey.currentState?.setFieldMandatory('customerName', true, index: 0);
  /// ```
  void setFieldMandatory(String fieldKey, bool isMandatory, {int? index}) {
    ...
    // Find and update the field's isMandatory property
    for (Section section in widget.sections) {
      for (RowElement row in section.rows ?? []) {
        for (DynamicField field in row.fields ?? []) {
          // Match either the base key or the flattened key
