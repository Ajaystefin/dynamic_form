...
void main() {
  ...
  setUp(() {
    ...
    // Mock local storage fully to avoid platform channel exceptions and let loginWithSSO succeed
  ...
  group('SplashViewModel Tests', () {
    ...
    test('Init with sso_error redirects to login and shows failure toast',
      ...
      verify(() => mockAlertManager
          .showFailureToast('auth.login.errorMessageSSO'.tr())).called(1);
      verify(() => mockRouter.go(Routes.login)).called(1);
    });
    ...
    test(
        'Init with invalid tokenResponse (missing userResponse) throws and goes to login',
        () async {
      EnvConfig.setConfigForTesting({'isSSOEnabled': true});
      when(() => mockRouter.go(Routes.login)).thenReturn(null);
      when(() => mockAlertManager.showFailureToast(any())).thenReturn(null);
      ...
      verify(() => mockAlertManager.showFailureToast(
          'auth.login.errorMessageInvalidResponse'.tr())).called(1);
      verify(() => mockRouter.go(Routes.login)).called(1);
    });
    ...
    test(
        'handleTokenFound successful login with NO roles throws and goes to login',
        () async {
      EnvConfig.setConfigForTesting({'isSSOEnabled': true});
      when(() => mockRouter.go(Routes.login)).thenReturn(null);
      when(() => mockAlertManager.showFailureToast(any())).thenReturn(null);
      ...
      verify(() => mockAlertManager
          .showFailureToast('auth.login.errorMessageNoRoles'.tr())).called(1);
      verify(() => mockRouter.go(Routes.login)).called(1);
    });
