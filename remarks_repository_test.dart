  ...
  group('RemarksRepository Integration Tests', () {
    ...
    group('getFinancialDetailsFromCreditLens', () {
      test('returns FinancialDetailsResponse on success', () async {
        ...
        expect(mockAPIManager.callLog[0]['body'],
    ...
    group('deleteGuarantorDetailsByEntityId', () {
      test('returns DeleteFinancialRatioAnalysisResult on success', () async {
        final innerJson = 'Success';
        final fullBody = {'responseData': innerJson};
        final mockResponse = AppResponse(
          message: 'OK',
          code: 200,
        ...
        final result =
            await repository.deleteGuarantorDetailsByEntityId(entityId: 999);

        expect(result, isA<DeleteFinancialRatioAnalysisResult>());
        expect(result.message, equals('Success'));
        expect(mockAPIManager.callLog, hasLength(1));
        expect(mockAPIManager.callLog[0]['endpoint'],
            equals(APIEndpoints.deleteGuarantorDetailsByEntityId));
        expect(mockAPIManager.callLog[0]['body'],
            containsPair('requestData', {'entityId': '999'}));
      });

      test('throws when API returns error status', () async {
        final mockError = AppResponse(
          message: 'Error deleting by entity ID',
          code: 500,
          status: ResponseStatus.error,
          body: <String, dynamic>{},
        );
        ...
        expect(
          () => repository.deleteGuarantorDetailsByEntityId(entityId: 999),
          throwsA('Error deleting by entity ID'),
        );
      });
    });
  });
