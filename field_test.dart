...
void main() {
  ...
  group('Field Dependencies Tests', () {
    testWidgets('creates controller for dependent textField', (tester) async {
      final dependencies = FieldDependencies([
        FieldDependency(
      ...
      expect(controllers.containsKey('fullName'), true);
    });

    testWidgets('creates controller for dependent percentage field',
        (tester) async {
      final dependencies = FieldDependencies([
        FieldDependency(
          dependentFieldKey: 'totalPercent',
          sourceFieldKeys: ['percent1', 'percent2'],
          calculator: (doc) {
            final p1 = double.tryParse(doc['percent1']?.toString() ?? '0') ?? 0;
            final p2 = double.tryParse(doc['percent2']?.toString() ?? '0') ?? 0;
      ...
      expect(controllers.containsKey('totalPercent'), true);
    });

    testWidgets('creates controller for dependent customerSearch field',
        (tester) async {
      final dependencies = FieldDependencies([
        FieldDependency(
          dependentFieldKey: 'searchKey',
          sourceFieldKeys: ['customerId'],
          calculator: (doc) => doc['customerId']?.toString() ?? '',
        ),
      ]);
      ...
      final field = DynamicField(
        controlType: FieldType.customerSearch,
        key: 'searchKey',
        label: 'Search',
        required: false,
        rowData: 1,
      ...
      expect(controllers.containsKey('searchKey'), true);
    });

    testWidgets('creates controller for dependent amount field',
        (tester) async {
      final dependencies = FieldDependencies([
        FieldDependency(
          dependentFieldKey: 'totalAmount',
          sourceFieldKeys: ['amount1', 'amount2'],
          calculator: (doc) {
            final a1 = double.tryParse(doc['amount1']?.toString() ?? '0') ?? 0;
            final a2 = double.tryParse(doc['amount2']?.toString() ?? '0') ?? 0;
      ...
      expect(controllers.containsKey('totalAmount'), true);
    });
  ...
  group('Inactive Field Tests', () {
    testWidgets(
        'renders SizedBox when field is not active and not enabledDefault',
        (tester) async {
      final field = DynamicField(
        controlType: FieldType.textField,
        key: 'inactive_key',
