...
void main() {
  ...
  late CountrySummaryViewModel viewModel;
  ...
  setUp(() async {
    ...
    viewModel = CountrySummaryViewModel()
      ..repository = mockRepo
  ...
  testWidgets('changeTab() emits loading then loaded with new tab',
      (tester) async {
    const newTab = CountrySummaryTabs.fiRecommend;
  ...
  test('getTabLabel() returns translation key', () {
    const tab = CountrySummaryTabs.rational;
    final key = TabConstants.countrySummaryTitles[tab]!;
  ...
  group('init()', () {
    ...
    test('preserves activeTab during initialization', () async {
      viewModel.changeTab(CountrySummaryTabs.summaryOfLatestDev);
  ...
  group('getTabLabel() comprehensive tests', () {
    test('returns correct labels for all CountrySummaryTabs', () {
      for (final tab in CountrySummaryTabs.values) {
        final label = viewModel.getTabLabel(tab);
        expect(label, isNotNull);
        expect(label, isNotEmpty);
        expect(TabConstants.countrySummaryTitles.containsKey(tab), true);
      }
    });

    test('label changes when switching tabs', () {
      const tab1 = CountrySummaryTabs.bankingSector;
      const tab2 = CountrySummaryTabs.fiRecommend;
  ...
  group('State management edge cases', () {
    ...
    test('state copyWith method works correctly', () {
      const initialState = CountrySummaryState(
  ...
  group('Simplified coverage tests', () {
    ...
    test('saveComment creates and saves comment correctly', () async {
      // Arrange
      const testComment = 'Test Country summary comment';
    ...
    test('changeTab simple functionality', () async {
      viewModel.changeTab(CountrySummaryTabs.bankingSector);
      await Future.delayed(const Duration(milliseconds: 1100));
