...
// Please enter
  ...
  /// InputFormatter that allows typing/deleting “N/A” and numeric values
  /// with up to two decimal places.
  ///       otherwise only allow up to 3 digits, optional "." + up to 6 digits
  static TextInputFormatter naOrNumberUpTo100Formatter() {
    return TextInputFormatter.withFunction((oldValue, newValue) {
      final txt = newValue.text;
      if (txt.isEmpty) return newValue;
      if (RegExp(r"^[Nn\/Aa]{0,15}$").hasMatch(txt)) {
        return newValue;
      }
      final numericPattern = RegExp(r"^\d{0,15}(\.\d{0,6})?$");
      if (numericPattern.hasMatch(txt)) {
        return newValue;
      }
      return oldValue;
    });
...
class DecimalInputFormatter extends TextInputFormatter {
  DecimalInputFormatter({this.regEx});
  RegExp? regEx;
...
class RegexInputFormatter extends TextInputFormatter {
  ...
  TextEditingValue formatEditUpdate(
    ...
    // Check if new text matches pattern
    if (_regExp.hasMatch(newValue.text)) {
      return newValue;
    }
...
//For Project contract Numeric 21+6 decimal - now use 15+6
class NumericDecimalTextInputFormatter extends TextInputFormatter {
  ...
  TextEditingValue formatEditUpdate(
    ...
    // Enforce max digits
    if (integerPart.length > maxIntegerDigits) return oldValue;
