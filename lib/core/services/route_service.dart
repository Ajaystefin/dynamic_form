...
import "package:wcas_frontend/features/admin/workflow_configuration/view.dart";
...
import "package:wcas_frontend/features/auth/splash/view.dart";
...
import "package:wcas_frontend/features/request/approval/credit_assessment_fi/view.dart";
...
import "package:wcas_frontend/features/request/approval/previous_credit_approval/view.dart";
...
import "package:wcas_frontend/features/request/approval/recommend_for_current_approval/view.dart";
...
import "package:wcas_frontend/features/request/ccsys/termination/view.dart";
...
import "package:web/web.dart"
    if (dart.library.io) "package:wcas_frontend/core/services/web_stub.dart"
    as web;

/// Application Router
///
/// Configures application navigation, authentication redirects,
/// route observers, error handling, and all registered routes
...
GoRouter router = GoRouter(
  ...
  redirect: (context, state) async {
    ...
    final bool isLoggedIn = await AuthRepository.instance.isLoggedIn();
    final String path = state.uri.path;
    final bool isNewSession = checkAndMarkSession();
    logger.i(
      "Router Check: isLoggedIn=$isLoggedIn, path=$path, "
      "Routes.login=${Routes.login} isNewSession=$isNewSession",
    );
    if (!isLoggedIn || isNewSession) {
      final normalizedPath = path.replaceAll("//", "/");
      if (normalizedPath == Routes.login ||
          normalizedPath == Routes.splash ||
          normalizedPath == Routes.loginSuccess ||
          normalizedPath == Routes.ssoFailed ||
          normalizedPath == Routes.logout ||
          normalizedPath == "/debug") {
        return null; // Stay on login or splash page
      }
  ...
  routes: [
    ...
    GoRoute(
      ...
      builder: (context, state) {
        ...
        return SplashView(queryParams: queryParams);
      },
    ),
    GoRoute(
      name: Routes.loginSuccess,
    ...
    GoRoute(
      name: Routes.ssoFailed,
      path: "/${Routes.ssoFailed}",
      builder: (context, state) {
        final queryParams = {...state.uri.queryParameters, "sso_error": "true"};
        return SplashView(queryParams: queryParams);
      },
    ),
    GoRoute(
    ...
    GoRoute(
      ...
      builder: (context, state) => const OtherCertificationsView(),
    ),
    GoRoute(
    ...
    GoRoute(
      name: Routes.workflowConfiguration,
      path: "/${Routes.workflowConfiguration}",
      builder: (context, state) => const WorkflowConfigurationView(),
    ),
    GoRoute(
    ...
    GoRoute(
      ...
      builder: (context, state) {
        final ApplicationFilterType filterType =
    ...
    GoRoute(
      ...
      builder: (context, state) {
        ...
        return CreateSecurityView(
          security: args["security"] as Security?,
    ...
    GoRoute(
      name: Routes.previousCreditApproval,
      path: "/${Routes.previousCreditApproval}",
      builder: (context, state) => const PreviousCreditApprovalView(),
    ),
    GoRoute(
      name: Routes.recommendationCurrentApproval,
      path: "/${Routes.recommendationCurrentApproval}",
      builder: (context, state) => const RecommendCurrentApprovalView(),
    ),
    GoRoute(
    ...
    GoRoute(
      name: Routes.ccsysTerminateWithdraw,
      path: "/${Routes.ccsysTerminateWithdraw}",
      builder: (context, state) => const CcsysTerminationView(),
    ),
    GoRoute(
    ...
    GoRoute(
      ...
      builder: (context, state) {
        final extras = state.extra as Map<String, dynamic>?;
        final Contract? contract = extras?["contract"] as Contract?;
        final Project? project = extras?["project"] as Project?;
    ...
    //     return LinkContractView(projectItem: project);
    //   },
    ...
    GoRoute(
      ...
      builder: (context, state) {
        ...
        // Support BOTH shapes:
        //  - legacy:   extra is Project?
        //  - new map:  { 'project': Project, 'facilityArgs':
        // CreateFacilityArgs? }
        final Project? project =
            (extra is Map) ? extra["project"] as Project? : extra as Project?;
    ...
    GoRoute(
      ...
      builder: (context, state) {
        final project = state.extra as Project?;
        return CreateProjectView(isCreateProject: false, projectItem: project);
      },
    ),
    GoRoute(
    ...
    GoRoute(
      name: Routes.creditAssessmentFI,
      path: "/${Routes.creditAssessmentFI}",
      builder: (context, state) => const CreditAssessmentFIView(),
    ),
  ],
);
...
///
/// Provides convenient access to the current route location
/// from the active [GoRouter] configuration.
extension GoRouterLocation on GoRouter {
  /// Returns the current route location as a URI string.
  String get location {
...
/// Go Router Observer
///
/// Tracks route navigation events and updates the current and previous
/// route names in [Globals] whenever navigation occurs.
class GoRouterObserver extends NavigatorObserver {
  ...
  void didPush(Route<dynamic> route, Route<dynamic>? previousRoute) {
    ...
    logger.i(
      "Route pushed: ${Globals.previousRoute} -> ${Globals.currentRoute}",
    );
  }
  ...
  void didPop(Route<dynamic> route, Route<dynamic>? previousRoute) {
    ...
    Globals.previousRoute = previousRoute?.settings.name ?? "/";
    logger.i(
      "Route popped: ${Globals.previousRoute} -> ${Globals.currentRoute}",
    );
  }
  ...
  void didRemove(Route<dynamic> route, Route<dynamic>? previousRoute) {
    ...
    Globals.previousRoute = previousRoute?.settings.name ?? "/";
    logger.i(
      "Route removed: ${Globals.previousRoute} -> ${Globals.currentRoute}",
    );
  }
  ...
  void didReplace({Route<dynamic>? newRoute, Route<dynamic>? oldRoute}) {
    ...
    logger.i(
      "Route replaced: ${Globals.previousRoute} -> ${Globals.currentRoute}",
    );
  }
}

/// Checks if a session is active in the current tab.
///
/// Returns [`true`] if this is a new session (opened in new tab).
/// Returns [`false`] if the session is already active (refresh).
bool checkAndMarkSession() {
  try {
    final storage = web.window.sessionStorage;
    if (storage.getItem("wcas_session_active") == null) {
      storage.setItem("wcas_session_active", "true");
      return true; // New session
    }
    return false; // Existing session (Refresh)
  } on Object {
    // Fallback for non-web environments or errors
    return false;
  }
}
