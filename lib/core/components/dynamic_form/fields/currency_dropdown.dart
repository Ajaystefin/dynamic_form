...
class DynamicFormCurrencyDropdownTextfield extends StatefulWidget {
  /// Creates a [DynamicFormCurrencyDropdownTextfield].
  ...
  /// Callback invoked when the currency value changes.
...
class _DynamicFormCurrencyDropdownTextfieldState
  ...
  // Get the raw (unrounded) amount from document
  double? _getInitialAmount() {
    final CurrencyFieldData? data = CurrencyUtils.extractFromDocument(
      widget.document,
      widget.fieldData.key,
    );
  ...
  Widget build(BuildContext context) {
    ...
    final Widget child = CurrencyDropdown(
      ...
      initialAmount: _getInitialAmount(),
...
class CurrencyDropdown extends StatefulWidget {
  ...
  /// Initial AED equivalent value.
  final double? initialAedEquivalent;

  /// Raw (unrounded) amount from the document. The amount field displays a
  /// rounded version of this, so it is kept here to emit the original value
  /// back to the document when the user has not edited the field.
  ...
  /// Indicates whether the amount field is read-only.
  final bool readOnly;

  /// Indicates whether the currency dropdown is disabled.
...
class _CurrencyDropdownState extends State<CurrencyDropdown> {
  /// How long to wait after the last trigger before fetching the rate.
  static const Duration _rateDebounceDuration = Duration(milliseconds: 300);
  ...
  Completer<void>? _rateDebounceCompleter;
  int _rateRequestId = 0;

  /// True while a rate fetch is pending (debouncing) or in flight. Drives the
  /// loader in the AED field.
  final ValueNotifier<bool> _isFetchingRate = ValueNotifier<bool>(false);
  ...
  void dispose() {
    ...
    _releasePendingRateDebounce();
    _isFetchingRate.dispose();
    aedController.dispose();
    super.dispose();
  }

  /// Debounced entry point for [getCurrencyRates].
  ///
  /// Collapses rapid triggers (typing in the amount field) into a single call
  /// [_rateDebounceDuration] after the last one. A superseded caller's future
  /// completes immediately so its trailing work is never stranded.
  Future<void> _debouncedGetCurrencyRates(Reference? selectedCurrency) {
    _rateDebounceTimer?.cancel();
    _releasePendingRateDebounce();

    final Completer<void> completer = Completer<void>();
    final int requestId = ++_rateRequestId;
    _rateDebounceCompleter = completer;
    _isFetchingRate.value = true;

    _rateDebounceTimer = Timer(_rateDebounceDuration, () async {
      await getCurrencyRates(selectedCurrency, requestId: requestId);

      if (identical(_rateDebounceCompleter, completer)) {
        _rateDebounceCompleter = null;
      }
  ...
  /// Completes the pending debounce future, if any, without running its fetch.
  void _releasePendingRateDebounce() {
    final Completer<void>? pending = _rateDebounceCompleter;
    _rateDebounceCompleter = null;
  ...
  /// The amount to emit back to the document.
  ///
  /// The amount field shows a rounded value, so re-parsing its text would push
  /// that rounding into the data. When the text still matches the formatted
  /// [CurrencyDropdown.initialAmount] the user has not edited it, so the
  /// original unrounded value is emitted instead.
  double _amountForEmit() {
    final String text = widget.controller?.text ?? "";

    if (widget.initialAmount != null &&
        CurrencyUtils.formatCurrency(widget.initialAmount!) == text) {
      return widget.initialAmount!;
    }

    return CurrencyUtils.parseCurrency(text) ?? 0;
  }
    ...
    if (widget.initialOption?.key != oldWidget.initialOption?.key) {
      ...
      if (widget.initialOption != null &&
        ...
        WidgetsBinding.instance.addPostFrameCallback((_) async {
          ...
          await _debouncedGetCurrencyRates(
          ...
          if (widget.onChanged != null) {
            final double numValue = _amountForEmit();
        ...
        children: [
          CustomTextField(
            prefixIcon: CustomDropdownButton(
              // No key here: a ValueKey based on the currency used to force
              // this widget to be disposed and recreated on every change,
              // which raced with dropdown_search's own dispose()->
              // closePopup() while its popup was still closing and caused
              // an extra Navigator.pop() to land on the page route. Selection
              // now syncs in place via didUpdateWidget in CustomDropdownButton.
              borderRadius: 4,
          ...
          if (selectedOption != null &&
            ...
            CustomTextField(
              ...
              // Grey loader shown while an exchange rate fetch is pending
              // (debouncing) or in flight. The SizedBox is always rendered so
              // the field does not shift when the loader appears.
              suffixIcon: ValueListenableBuilder<bool>(
                valueListenable: _isFetchingRate,
                builder: (context, isFetchingRate, _) => Padding(
  ...
  /// has been scheduled in the meantime, so a slow response for a currency the
  /// user has already moved on from cannot overwrite the current value.
