...
/// Dynamic grid/table form field.
class DynamicFormGrid extends StatefulWidget {
  /// Creates a [DynamicFormGrid].
  ...
  /// Indicates whether the grid should be rendered as a table.
  final bool isTable;
  ...
  /// Callback invoked when a controller is created.
      ...
      final List<Widget> rowWidgets = [
        for (int i = 0; i < dynamicFormGridFields.length; i++)
          buildGridCellWidget(
            ...
            i,
            rowIndex,
            isNewRow: false, // Load from document
          ),
      ];
        ...
        rowWidgets.add(
          Builder(
            builder: (ctx) => Visibility(
              visible: rowIndex != 0 && !FormAccessProvider.of(ctx),
              child: deleteFunction(),
            ),
          ),
        );
      }
        ...
        children: [
          Row(
            children: [
              ...
              if (!widget.isTable && !FormAccessProvider.of(context))
                IconButton(
                  tooltip: "common.addRow".tr(),
      ...
      case FieldType.textField || FieldType.accountNo:
        returnWidget = CustomTextField(
          ...
          validator: dynamicField.isRequired
              ? (value) {
                  final actualValue = controller.text;
                  if (actualValue.isEmpty) {
                    return dynamicField.message;
                  }
                  if (dynamicField.validationPattern != null) {
                    final pattern = RegExp(dynamicField.validationPattern!);
                    if (!pattern.hasMatch(actualValue)) {
                      return dynamicField.message;
                    }
                  }
                  return null;
      ...
      case FieldType.singleCheckBox:
        {
          ...
          final bool boolValue = currentValue is bool && currentValue;
          ...
          returnWidget = DynamicFormSingleCheckBox(
            ...
            onSaved: ({value}) {
              widget.document[controllerKey] = value ?? false;
            },
          );
          break;
        }
