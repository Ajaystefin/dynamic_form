...
//filter fucntion for Row as List<Widget>
...
/// filter fucntion for Row as RowModel
List<RowModel> addFilterForRowModel(
  ...
  List<RowModel> finalRow = [];
  if (filterRow.widget.isNotEmpty) {
    rowsPerPage += 1;
  }
  for (var i = 0; i < rows.length; i++) {
    if (i % rowsPerPage == 0) {
...
class CustomRawTable extends StatefulWidget {
  ...
  ///[rows] are for simple Rows in the table
  final List<List<Widget>>? rows;

  ///[rowModels] contains (List<List<Widget>>? and Color?), Can Update Row Color to the Each Rows
  final List<RowModel>? rowModels;

  ///[autoFitWidth] make the table width fits automatically to the respective screen width
  ///If [autoFitWidth] is false then we can control [columnSpacing] the width manually,
  ///But even if we assign column width and [autoFitWidth] is true, That [columnSpacing] won't be considered
  ///
...
class _CustomRawTableState extends State<CustomRawTable> {
  ...
  void initState() {
    ...
    if (widget.topStackedHeaders?.isNotEmpty ?? false) {
      _mapStackedMainHeadersToColumns(
          columns: widget.columns, headers: widget.topStackedHeaders ?? []);
    }

    if (widget.stackedHeaders?.isNotEmpty ?? false) {
      _mapStackedHeadersToColumns(
          columns: widget.columns, headers: widget.stackedHeaders ?? []);
    }
  ...
  /// - [defaultRowsPerPage]: Fallback value if `rowsPerPage` is null (default is 10).
  ///
  /// Returns:
  ...
  Widget build(BuildContext context) {
    ...
    final List<RowData> visibleRows = widget.rowsPerPage != null
        ? _filteredRows.sublist(startIndex,
                ...
                child: Container(
                  decoration: BoxDecoration(
                      border: (widget.stackedHeaders?.isNotEmpty ?? false)
                          ? Border.all(color: AppColors.tableActivatedColor)
                          : null),
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                            ...
                            children: List.generate(
                              ...
                              (int index) {
                                final bool isStackedHeader =
                                    stackedMainHeaders[index].startIndex != -1;
                                Color? bordorColor =
                                ...
                                return Container(
                                  ...
                                  height: isMobile
                                      ? mobileViewHeight
                                      : widget.topStackedHeaderHeight ?? 30.w,
                                  decoration: BoxDecoration(
                                    color: widget.headerColor ??
                                        AppColors.tableHeadingColor,
                                    border: Border.symmetric(
                                      vertical: BorderSide(
                                          color: bordorColor, width: 0.5),
                                  ...
                                  child: Center(
                                      child: stackedMainHeaders[index].widget),
                                );
                              },
                            ),
                          ),
                            ...
                            children: List.generate(
                              ...
                              (int index) {
                                ...
                                return Container(
                                  ...
                                  height: isMobile
                                      ? mobileViewHeight
                                      : widget.stackedHeaderHeight ?? 30.w,
                                  decoration: BoxDecoration(
                                    color: widget.headerColor ??
                                        AppColors.tableHeadingColor,
                                    border: Border.symmetric(
                      ...
                      tableColumnWidget(
                        ...
                        child: DataTable(
                          columnSpacing: 0,
                          headingRowHeight: widget.columnHeaderHeight ?? 40,
                          horizontalMargin: 0,
                          ...
                          columns: [
                            ...widget.columns.asMap().entries.map((entry) {
                              ...
                              return TableColumn(
                                  ...
                                  child: Flexible(
                                    child: Padding(
                                      padding: const EdgeInsets.symmetric(
                                          horizontal: 4.0),
                                      child: Column(
                                        mainAxisAlignment: hasStackedHeaders
                                            ? (column.isStacked
                                ...
                                // isMobile
                                //     ? FixedColumnWidth(mobileViewWidth)
                                //     : column.forcedWidth != null
                                //         ? FixedColumnWidth(column.forcedWidth!)
                                //         : (!hasStackedHeaders &&
                                //                 widget.autoFitWidth)
                                //             ? null
                                //             : column.width != null
                          ...
                          rows: _filteredRows.isEmpty
                              ...
                              : visibleRows.asMap().entries.map((entry) {
                                  ...
                                  return DataRow(
                                    ...
                                    color: WidgetStatePropertyAll(row.rowColor),
                                    cells: [
                                      ...row.cells
                                          .asMap()
                                          .entries
            ...
            children: [
              ...
              Row(
                ...
                children: [
                  ...
                  Text(
                    'Showing ${startIndex + 1}-${endIndex > totalCountWOFilter.length ? totalCountWOFilter.length : endIndex} of ${totalCountWOFilter.length}',
