...
class CustomerRepository {
  static final _singleton = CustomerRepository();
  ...
  Future<List<Country>?> getCountries() async {
    try {
      ...
      if (response.status == ResponseStatus.success &&
        ...
        List<dynamic> res = response.body["responseData"];
  ...
  Future<Customer?> searchUserDetails(String? customerRimNo,
      String? customerName, String? groupId, String? groupName) async {
  ...
  Future<Customer?> searchUserDetailsForCL(String? customerRimNo,
      String? customerName, String? groupId, String? groupName) async {
    Customer? requestCustomer;
    try {
      Map data = BaseRequest.baseRequest({
      ...
      if (response.status == ResponseStatus.success) {
        if (response.body["responseData"] != null &&
          ...
          try {
            requestCustomer =
                Customer.fromJson(response.body["responseData"]["PartyInfo"]);
          } catch (e) {
            AlertManager().showFailureToast(e.toString());
          }
        }
  ...
  Future<List<Customer?>> searchCustomerProfile(
      String? customerName, String? groupId, String? groupName,
    ...
    try {
      ...
      if (groupName != null) {
        ...
        for (Customer? customer in resultCustomers) {
          ...
          if (groupId != null) {
            ...
            // If no group ID, keep the customer (use a unique key based on customer ID or index)
  ...
  Future<String?> saveUserDetails(
      ...
      List<CustomerException>? customerException) async {
    try {
      Map data = BaseRequest.baseRequest({
  ...
  Future<List<CustomerOwnerShipInfo>?> getCustomerInformationByRimOwnership(
      int? customerRimNo) async {
    try {
      Map data = BaseRequest.baseRequest({"custInfoId": customerRimNo});
      AppResponse response = await _apiManager.post(
          APIEndpoints.getCustomerInformationByRimOwnership, data);
  ...
  Future<List<CustomerException>?> getCustomerInformationByRimException(
    ...
    try {
      Map data = BaseRequest.baseRequest({"custInfoId": customerRimNo});
      AppResponse response = await _apiManager.post(
          APIEndpoints.getCustomerInformationByRimException, data);
      if (response.status == ResponseStatus.success &&
          response.body["responseData"] != null) {
        List<dynamic>? res = response.body["responseData"];
        return (res ?? [])
  ...
  Future<String> getBorrowerCustomers(
      List<Customer?> customerList, Customer primaryCustomer) async {
    try {
      Map data = BaseRequest.baseRequest({
        "partyIds":
      ...
      if (response.status == ResponseStatus.success) {
        ...
        for (Customer borrower in Globals.request!.borrowers!) {
          if (borrower.customerRimNo == primaryCustomer.customerRimNo) {
            borrower.isPrimary = true;
          }
        }
        for (Customer borrower in Globals.request!.nonBorrowers!) {
          if (borrower.customerRimNo == primaryCustomer.customerRimNo) {
            borrower.isPrimary = true;
  ...
  Future<List<Customer>?> getChildRimsForGroup() async {
    try {
      ...
      if (response.status == ResponseStatus.success &&
          response.body["responseData"] != null) {
        List<dynamic> res = response.body["responseData"];
        return res.map((item) => Customer.fromJson(item)).toList();
      } else {
        logger.i("countries--: ${response.message}");
        throw response.message;
      }
...
// Do not delete or change because its using for customer Information
  Future<Customer?> searchUserDetailsPartyInqOnly(String customerRimNo,
      String customerName, String groupId, String groupName) async {
    Customer? requestCustomer;
    try {
      AppResponse response = await _apiManager.post(APIEndpoints.searchPartyInq,
          getPayload(customerRimNo, customerName, groupId, groupName));
      if (response.status == ResponseStatus.success) {
        if (response.body["partyInqOwnershipDTO"]["PartyInqRs"]["PartyRec"] !=
            null) {
          try {
            requestCustomer = Customer.fromJson(response
                .body["partyInqOwnershipDTO"]["PartyInqRs"]["PartyRec"]);
          } catch (e) {
            logger.e(e.toString());
          }
        }
  ...
  Map getPayload(String? customerRimNo, String? customerName, String? groupId,
      String? groupName) {
