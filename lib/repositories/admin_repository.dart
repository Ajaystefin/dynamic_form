...
class AdminRepository {
  ...
  Future<String?> saveAccessRights(
      AccessRight accessRight, bool isUpdate) async {
    ...
    try {
      ...
      Map data = BaseRequest.baseRequest(accessRight.toJson());
  ...
  Future<AccessRight?> getAccessRights(
      Reference role, Reference requestType) async {
  ...
  Future<List<Reference>> getReferenceData(ReferenceType reference) async {
    ...
    if (response.status == ResponseStatus.success) {
      ...
      for (var item in (responseData as List)) {
  ...
  Future<String?> saveReferenceDataInformation(
    ...
    Map data = BaseRequest.baseRequest(reference?.toJson());
    log(json.encode(data).toString());

    AppResponse response = await APIManager.instance
  ...
  Future<User> getUserDetailList(User? userListItem) async {
    ...
    List<Reference> roleTypes = referenceData[ReferenceDataKeys.roleType] ?? [];
    ...
    try {
      ...
      List rawRoleList = userData.remove('roleList') ?? [];

      user = User.fromJson(userData);

      user.availableRoles = rawRoleList.map((roleCode) {
        roleCode = roleCode.trim();
        Reference roleReference = roleTypes.firstWhere(
  ...
  Future<String?> saveUserDetailsList(
    ...
    try {
      Map data = BaseRequest.baseRequest(userDetails?.toSaveDetailsJson());
  ...
  Future<List<FileAccess>> getFileAttachments(Reference? role) async {
    Map data = BaseRequest.baseRequest({"role": role?.reference1});
  ...
  Future<String?> saveFileAttachments(
      List<FileAccess> fileAccesses, Reference? role) async {
    List<FileAccess> updatedFileAccesses =
