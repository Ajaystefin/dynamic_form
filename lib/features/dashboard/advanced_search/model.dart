...
/// View model for advanced search screen.
class AdvancedSearchViewModel extends SafeCubit<AdvancedSearchState> {
  /// Creates an [AdvancedSearchViewModel].
  ...
  /// Application reference number search value.
  String? applicationRefNo;

  /// Customer RIM number search value.
  String? customerRimNo;

  /// Group ID search value.
  String? groupId;

  /// Selected regions for advanced search.
  List<Reference>? selectedRegions;

  /// Selected segments for advanced search.
  ...
  /// Selected application type filter data.
  List<Request> selectedFilterTypeData = [];

  /// Selected role references.
  List<Reference>? selectedRoles;

  /// Role list with mapped users.
  List<Role> users = [];

  /// User models used for filtered users.
  List<User> userModels = [];

  /// User list used in dropdowns.
  List<User> userList = [];
  ...
  /// Worklist search result.
  List<Request> workList = [];

  /// Filtered worklist search result.
  List<Request> filteredWorkList = [];

  /// Form key used for advanced search validation.
  final GlobalKey<FormState> formKey = GlobalKey<FormState>();

  /// Reference data used by advanced search fields.
  Map<String, List<Reference>> referenceData = {};

  /// Request reference number table filter.
  String? reqRefNoFilter;

  /// Request type table filter.
  Reference? requestTypeFilter;

  /// Applicant RIM table filter.
  String? applicantRimFilter;

  /// Applicant name table filter.
  ...
  /// Opens the selected application from the worklist.
  ...
  /// Handles advanced search submit button press.
  Future<void> onSubmitButtonPress() async {
    try {
      ...
      AlertManager().showFailureToast("Error: $e");
    }
  }
  ...
  void onFilter({
    ...
    filteredWorkList = workList.where((data) {
      // Reference Number
      if ((reqRefNoFilter?.isNotEmpty ?? false) &&
      ...
      // Applicant RIM
      if ((applicantRimFilter?.isNotEmpty ?? false) &&
      ...
      // Applicant Name
      if ((applicantNameFilter?.isNotEmpty ?? false) &&
          !(data.customerName ?? "")
              .toLowerCase()
  ...
  List<Reference>? getRoleList() {
    return referenceData[ReferenceDataKeys.roleType]
        ?.where(
          (element) =>
              (element.id != ServerConstants.admId) &&
              (element.id != ServerConstants.inqusrId) &&
              // (element.id != ServerConstants.lmtId) && //CCU-Maker
              (element.reference2 != ServerConstants.approvalDelegation),
        )
        .toList();
  }
  ...
  /// Gets filtered users based on the selected filter id.
  ...
  /// Returns request status name by request status id.
