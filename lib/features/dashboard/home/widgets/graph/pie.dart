...
/// Displays the dashboard ageing summary as a pie chart.
class PieChartView extends StatefulWidget {
  /// Creates a [PieChartView].
  const PieChartView(this.viewModel, {this.isMobile = false, super.key});

  /// Home dashboard view model used to provide chart data and filters.
  final HomeViewModel viewModel;

  /// Indicates whether the chart is displayed on a mobile layout.
  final bool isMobile;
...
class _PieChartViewState extends State<PieChartView> {
  ...
  List<Widget> graphWidgets({bool isMobile = false}) {
    ...
    final double above30Days =
        widget.viewModel.ageingSummary?.aboveThirtyDays ?? 0;
    final double totalCount = zeroToSevenDays +
        eightToFifteenDays +
        sixteenToThirtyDays +
        above30Days;
    return [
  ...
  Widget graphLegend({
    ...
    //final bool isSelected = type == widget.viewModel.selectedGraphFilter;
    final bool isSelected = widget.viewModel.isAgeingSelected &&
        type == widget.viewModel.selectedGraphFilter;

    //final Color? textColor = isSelected ? AppColors.teal : null;
    final Color textColor = isSelected ? AppColors.teal : AppColors.black;
    return Row(
