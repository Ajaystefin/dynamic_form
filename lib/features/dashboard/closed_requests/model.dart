...
/// View model for managing closed requests and CCSYS worklist data.
class ClosedRequestsViewModel extends SafeCubit<ClosedRequestsState> {
  /// Creates a [ClosedRequestsViewModel].
  ...
  /// Filtered closed request records displayed in the table.
  List<Request?> closedRequestFilteredData = [];

  /// Selected request type filters.
  List<Request> requestTypeFilter = [];

  /// Selected request status filters.
  List<Request> reqStatusFilter = [];

  /// Selected request status reference filters.
  List<Reference>? requestStatusFilter;

  /// Request status list used by filter dropdowns.
  ...
  /// Closed request records returned from the repository.
  List<Request> closedRequests = [];

  /// Reference data used by the closed requests screen.
  Map<String, List<Reference>> referenceData = {};

  /// Page heading displayed for the current closed request view.
  String pageHeading = "dashboard.closedRequests.title".tr();

  /// Whether the current view is CCSYS worklist.
  bool isCCSYS = false;

  /// CCSYS worklist data.
  List<Request> worklistData = [];

  /// Filtered CCSYS worklist data.
  ...
  String? getRequestStatusNameById(Object? id) {
  ...
  /// Sets the page heading based on the selected application filter type.
  void getHeading(ApplicationFilterType applicationType) {
    pageHeading = applicationTypes[applicationType] ?? "";
  }

  /// Mapping between application filter type and heading text.
...
// assign to the FIELD, not a local variable
  ...
  /// Returns application type reference data.
  ...
  /// Returns transaction type reference data.
        ...
        if (selectedTypes == null || selectedTypes.isEmpty) {
          ...
          for (final Request selectedType in selectedTypes) {
            final String selectedName =
                (selectedType.requestSubType?.name ?? "").trim();
            if (selectedName ==
                "dashboard.home.historicalApplicationTypes".tr()) {
              final String? dataName = data.requestSubType?.name?.trim();
              if (dataName != null &&
                  dataName.isNotEmpty &&
            ...
            } else if (data.requestSubType?.name?.trim() == selectedName) {
              requestTypeFilter.add(selectedType);
              typeMatch = true;
            }
          }
