...
/// ViewModel for managing the state and workflow logic of CCSYS approval.
class CcsysApprovalViewModel extends SafeCubit<CcsysApprovalState>
  ...
  /// Repository instance for handling CCSYS-related operations.
  ...
  /// Controller for the rich text editor used to enter CCSYS comments.
  final UnifiedEditorController controller = UnifiedEditorController();

  /// Scroll controller used by the CCSYS approval screen.
  final ScrollController scrollController =
      ScrollController(keepScrollOffset: false);

  /// List of CCSYS comments loaded for the request.
  List<Comment> comments = [];

  /// Current CCSYS comment.
  Comment? comment;

  /// Indicates whether RSA authentication is enabled.
  bool isRSAEnabled = false;

  /// Last assigned role information for the request.
  Role? lastAssignedRole;

  /// List of users available for recommendation actions.
  List<User> users = [];

  /// List of users available for return actions.
  List<User> usersReturn = [];

  /// Dropdown items for recommendation users.
  List<CustomDropdownItem> userList = <CustomDropdownItem>[];

  /// Dropdown items for return users.
  List<CustomDropdownItem> userListReturn = <CustomDropdownItem>[];

  /// Current user role.
  UserRole? userRole = Globals.user?.currentRole?.userRole;

  /// Current role code used for CCSYS mapping.
  String? roleCode = "";

  /// Current request details.
  Request requests = Globals.request!;

  /// Current application reference number.
  String? appRefNo = Globals.request?.applicationRefNo;

  /// Application type reference list.
  List<Reference> applicationType = [];

  /// CCSYS role type reference list.
  List<Reference> ccsysRoleType = [];

  /// CCSYS recommended roles reference list.
  List<Reference> ccsysRecommendenRolesList = [];

  /// CCSYS returned roles reference list.
  List<Reference> ccsysReturnedRolesList = [];

  /// Selected recommended user id.
  String? selectedUserId;

  /// Selected recommended user name.
  String? selectedUserName;

  /// Selected recommended user's BPM role.
  String? selectedUserBpmRole;

  /// Selected recommended user's BPM role code.
  String? selectedUserBpmRoleCode;

  /// Selected return user id.
  String? selectedReturnUserId;

  /// Selected return user name.
  String? selectedReturnUserName;

  /// Selected return user's BPM role.
  String? selectedReturnUserBpmRole;

  /// RSA token digits entered by the user.
  String rsaDigit = "";
  ...
  /// Current page mode derived from rights.
  PageMode pageMode = PageMode.na;

  /// Initializes rights and page mode for the given request.
  ...
  /// Module key used for saving and loading CCSYS draft data.
  @override
  String get draftModuleKey => DraftModuleKeys.ccsys;

  /// Form key used to uniquely identify the CCSYS approval draft.
  @override
  String get draftFormKey => Routes.ccsysApproval;

  /// Draft handler used to build and apply CCSYS approval draft data.
  @override
  ...
  /// Initializes repositories, reference data, users, comments, and draft data.
  ...
  /// Loads reference data required for CCSYS approval actions.
  ...
  /// Gets the last assigned role for the current CCSYS request.
  ...
  /// Gets users by roles for CCSYS approval.
  ...
  /// Fetches and sets CCSYS strategy comments for the current application.
    ...
    try {
      ...
      if (commentsd.isNotEmpty) {
        comments = commentsd
            ...
            .toList();
        logger.i("Strategy comment: ${comments[0].comment}");
  ...
  /// Saves the current CCSYS comments.
  ...
  /// Returns BPM role names from the given role list.
  ...
  /// Builds dropdown items from the given users.
  ...
  /// Sets the role code for the given user role.
  ...
  /// Indicates whether the recommend button should be shown.
  ...
  /// Indicates whether the approve button should be shown.
  bool get showApproveButton => Utils.checkRoles([
        UserRole.ccuMaker,
        UserRole.ccuChecker,
      ]);

  /// Indicates whether decline or cancel action should be shown.
  bool get showDeclineCancelButton => Utils.checkRoles([
        UserRole.ccuMaker,
        UserRole.ccuChecker,
      ]);

  /// Indicates whether the return button should be shown.
  ...
  /// Checks whether the given user role belongs to the credit control team.
...
// have code:
  ...
  /// Resolves a user role enum from the given role code.
  ...
  /// Checks whether the given role code belongs to the credit control team.
  ...
  /// Handles save, recommend, approve, cancel, and return actions.
  Future<void> onSavePress(BuildContext context, String action) async {
    try {
      if (action != "saveAndContinue") {
        ...
        //logger.i("-------$selectedUserBpmRoleCode");
        // bool isValid = false;
  ...
  /// Shows RSA authentication dialog and returns validation result.
  ...
  /// Validates the entered RSA token.
  ...
  /// Shows success confirmation dialog for the application action.
          ...
          text: (action == "recommend" && showRecommendButton)
              ? (checkRoleCreditControlTeam(selectedUserBpmRoleCode))
                  ...
                  : "ccsys.recommendationApproval.informationMsgRoleRO".tr(
                      namedArgs: {
                        "refno": appRefNo ?? "",
                        "userId": userName ?? userId ?? targetRole ?? "",
                      },
                    )
  ...
  /// Navigates to the next screen after action completion.
  void moveToNext() {
...
// --- ADD: resolve WCAS roles from reference mapping ---
  ...
  /// Builds recommended WCAS role values for the current logged-in user.
  ...
  /// Builds return WCAS role values for the current logged-in user.
  ...
  String _currentLoggedInRoleCode() {
    ...
    if (roles.isNotEmpty) {
      return _normalizeRoleToCode(roles.first.bpmRole);
    }
    return "";
  }
...
// --- ADD: load recommend users using WCAS roles mapping ---
  ...
  /// Loads recommendation users using WCAS role mapping.
  ...
  /// Loads return users using WCAS role mapping.
  Future<void> loadReturnUsers() async {
    ...
    //   final createdLabel = (createdRm?.trim().isNotEmpty ?? false)
  ...
  /// Returns initial comment value for the logged-in user's role and user id.
  String commentsInitialValue() {
    if (comments.isEmpty) {
      return "";
    }

    final String currentRoleCode = Globals.user?.currentRole?.code ?? "";
...
/// Extension methods for iterable helpers.
