...
/// Fields used to control CCSYS create request customer inputs.
enum ControlFields {
...
/// ViewModel for managing CCSYS create request form state and actions.
class CcsysCreateRequestViewModel extends SafeCubit<CcsysCreateRequestState> {
  ...
  /// Repository instance for handling customer-related operations.
  late CustomerRepository repository;

  /// Repository instance for handling CCSYS-related operations.
  late CcsysRepository repositoryCCSYS;

  /// Request object created for CCSYS request flow.
  late Request requestCreate;

  /// Focus node used by the create request form.
  final FocusNode formFocusNode = FocusNode();

  /// Application details used during CCSYS create request.
  ApplicationDetails? applicationDetails = ApplicationDetails();

  /// Field control notifier used to enable or disable customer fields.
  ...
  /// Entered or selected customer RIM number.
  String? customerRimNo;

  /// Entered or selected customer name.
  String? customerName;

  /// Selected customer branch name.
  String? branchName;

  /// Selected customer branch number.
  int? branchNo;

  /// Selected customer segment name.
  String? segmentName;

  /// Indicates whether the search is based on RIM.
  bool isRim = true;

  /// Indicates whether a customer search has already been performed.
  bool isSearched = false;

  /// Returns whether both customer RIM number and customer name are filled.
  ...
  /// Stores all customers used for filtering.
  List<Customer?> allCustomers = []; // Store all customers for filtering

  /// Selected button model state.
  ...
  /// Loading status for the customer RIM number field.
  LoadingStatus customerRimNoLoadingStatus = LoadingStatus.loaded;

  /// Loading status for the customer name field.
  LoadingStatus customerNameLoadingStatus = LoadingStatus.loaded;

  /// Indicates whether reset button was pressed.
  bool isResetPressed = false;

  /// List of customers from the current request.
  List<Customer> customers = [];

  /// Selected customer.
  Customer? customer;

  /// Reference data loaded for CCSYS create request.
  ...
  /// Loads reference data required for CCSYS create request.
  Future<void> getReferenceDatas() async {
    try {
      ...
      branchType = referenceData[ReferenceDataKeys.branchList] ?? [];
    } on Object catch (e) {
      e.toString();
  ...
  /// Updates field control state based on the active field and entered data.
  ...
  /// Cancels selected customer and resets customer selection state.
  ...
  /// Applies the selected customer from the selection dialog.
  void onSelectionPressed(BuildContext context, {bool closeDialog = false}) {
    ...
    if (selectedCustomer.value == null) {
      ...
      branchNo = customer?.branchNo ?? ServerConstants.defaultBranchNo;
      segmentName = customer?.segment ?? ServerConstants.defaultSegment;

      logger.i("customerRimNo $customerRimNo $customerName");
      emit(state.copyWith(loaderStatus: LoadingStatus.loaded));
    }
  }

  /// Filters customers in the selection dialog by customer name.
  ...
  /// Searches customer by RIM number or customer name.
  Future<void> onCustomerSearchPressed({
    ...
    try {
      ...
      if (isRim) {
        // RIM search
        customer = await repositoryCCSYS.searchUserDetails(
          customerRimNo ?? "",
          customerName ?? "",
        );
        ...
        final List<Customer?> resultCustomers =
            await repositoryCCSYS.searchCustomerProfile(customerName, "", "");
  ...
  Future<void> onSubmitButtonPress(BuildContext context) async {
    ...
    try {
      ...
      final String region = branchType
              .firstWhere(
                (e) => e.reference1 == branchNo?.toString(),
                orElse: () => Reference(
                  reference2: Globals.user?.regions?.first,
                ),
              )
              .reference2 ??
          "";
      ...
      requestCreate
        ...
        ..customerRimNo = rim
        ..branch = branchName
        ..region = region.isNotEmpty ? region : Globals.user?.regions?.first
  ...
  void onCustomerRimNoSearchPressed() {
    if ((customerRimNo ?? "").isNotEmpty && !isSearched) {
      ...
      onCustomerSearchPressed();
    }
  }

  /// Checks whether the current user belongs to other business roles.
  ...
  /// Checks whether the current user belongs to other credit control roles.
