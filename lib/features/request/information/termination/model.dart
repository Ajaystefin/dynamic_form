...
import "package:wcas_frontend/features/request/information/termination/state.dart";
...
/// View model responsible for managing the Termination screen.
///
/// Handles termination details, validation, draft management,
/// reference data loading, and termination-related business logic.
class TerminationViewModel extends SafeCubit<TerminationState>
    with DraftMixin<TerminationViewModel> {
  /// Creates a [TerminationViewModel] with an initial loading state.
  ...
  /// Repository used to retrieve and manage termination-related
  /// request data.
  late RequestRepository repository;

  /// Focus node used to manage focus within the Termination form.
  FocusNode formFocusNode = FocusNode();

  /// Form key used for validating and managing the state of the
  /// Termination screen form.
  ...
  /// Indicates whether the current user has edit access to the
  /// termination workflow.
  bool isEdit =
      Globals.user?.currentRole?.rights?[RightConstants.terminateWithdrawal] ==
          AccessType.edit;

  // int? reasonId;

  /// Indicates whether the termination process completed successfully.
  bool isTerminationSuccess = false;

  /// Available reasons that can be selected for termination.
  List<Reference> reasonForTermination = [];

  /// Customer information associated with the current request.
  List<Response> customerInfo = [];

  /// Review comments retrieved for the current request.
  List<Comment>? getReviewComments = [];

  /// Comments maintained as part of the termination workflow.
  List<Comment> comments = [];

  /// Comment currently being created or updated.
  ...
  @override
  String get draftFormKey => Routes.terminateWithdraw;

  @override
  DraftHandler<TerminationViewModel> get draftHandler =>
      TerminationDraftHandler();

  /// Current mode of the Termination screen.
  ///
  /// Determines whether the user can view or edit
  /// termination details.
  PageMode? terminationPageMode;

  /// Indicates whether the Termination screen is editable.
  ///
  /// Returns `true` when the current page mode is set to edit.
  bool get canEdit =>
      terminationPageMode == PageMode.edit; //&& Utils.canEditApplication();
  ...
  Future<void> init(BuildContext context) async {
    ...
    terminationPageMode =
        AuthRepository.getPageMode(RightConstants.terminateWithdrawal);
    await Future.wait([
      getReviewCommentsReference(
  ...
  Future<void> onTerminateButtonPressed(BuildContext context) async {
    final isValid = !canEdit || (formKey.currentState?.validate() ?? false);
