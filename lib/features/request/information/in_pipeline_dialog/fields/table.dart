      ...
      default:
        return Column(children: [
          CustomRawTable(
            ...
            rows:
                List.generate(viewModel.pipelineRequestDetails.length, (index) {
                ...
                TextButton(
                  onPressed: () {
                    if (context.mounted) {
                      ...
                      Globals.request?.applicationRefNo = info.applicationRefNo;
                ...
                Center(
                  child: Text(
                    DateFormat('dd/MM/yyyy')
                        .format(DateTime.parse(info.creditAppDate.toString())),
                  ),
                ),
                Text(info.requestType.toString()),
                Text(info.purpose.toString()),
                CustomTooltip(
                  message: info.status ?? "",
                  child: Text(
  ...
  List<TableColumn> getTableColumns(InPipelineDialogViewModel viewModel) {
    return [
      TableColumn(
          forcedWidth: 130.w,
          label:
              Text('requestInformation.pipelineDialog.applicationRefNo'.tr())),
      TableColumn(
          forcedWidth: 80.w,
