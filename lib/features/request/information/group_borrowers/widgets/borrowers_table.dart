...
/// Displays the borrowers table on the Group Borrowers screen.
///
/// Presents borrower information in a tabular format and supports
/// reviewing borrower details associated with the current request.
class BorrowersTable extends StatelessWidget {
  /// Creates a [BorrowersTable].
  ...
  Widget build(BuildContext context) {
    ...
    final List<Customer> borrowers = viewModel.isReadOnly
        ? (Globals.request?.groupBorrowers ?? [])
        : viewModel.borrowersList;
                    ...
                    onChanged: viewModel.isReadOnly
                        ...
                        : (bool? newValue) {
                            viewModel.toggleBorrowerExclusion(
                              rim ?? 0,
                              selectedForExclusion: newValue ?? false,
                            );
                          },
                  );
                } else {
