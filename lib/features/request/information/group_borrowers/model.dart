...
/// View model responsible for managing the Group Borrowers screen.
///
/// Handles borrower retrieval, borrower selection, inclusion and
/// removal of borrowers, validation, and navigation throughout
/// the group borrower workflow.
class GroupBorrowersViewModel extends SafeCubit<GroupBorrowersState> {
  /// Creates a [GroupBorrowersViewModel] with an initial loading state.
  ...
  /// Repository used to retrieve and manage borrower-related data.
  ...
  /// Customer name returned from the most recent RIM search.
  String? searchedCustomerName;

  /// Customer RIM number returned from the most recent RIM search.
  ...
  Future<void> fetchCustomersList() async {
    ...
    for (final dynamic borrowerEntry in Globals.request?.borrowers ?? []) {
      ...
      if (borrowerRim != null) {
        customersByRim[borrowerRim] = borrowerEntry;
      }
    }
  ...
  Future<void> searchCustomerByRim(String rimInput) async {
    ...
    try {
      final GroupBorrowerSearchResponse borrowerSearchResponse =
          await repository.getCustomerByRim(rimAsInt);

      if (borrowerSearchResponse.responseData != null) {
        ...
        customer = Customer(
          ...
          groupId: customer?.groupId ?? Globals.request?.groupId ?? 0,
          groupName: customer?.groupName ?? Globals.request?.groupName ?? "",
        );
  ...
  void includeSelectedBorrowers() {
    try {
      ...
      emit(const GroupBorrowersState(loaderStatus: LoadingStatus.loaded));
    } on Object catch (error) {
      AlertManager().showFailureToast(error.toString());
    }
  }
  ...
  /// (UI only).
  void toggleBorrowerStatus(int rimNumber, {required bool newIsBorrowerValue}) {
    _toggledBorrowerStatus[rimNumber] = newIsBorrowerValue;
    emit(state.copyWith(loaderStatus: LoadingStatus.loaded));
  }

  /// Toggle inclusion selection (checkbox) for a given [rimNumber].
  void toggleBorrowerSelection(
    int rimNumber, {
    required bool selectedForInclusion,
  }) {
  ...
  void toggleBorrowerExclusion(
    int rimNumber, {
    required bool selectedForExclusion,
  }) {
  ...
  void addPotentialBorrower({BuildContext? context}) {
    ...
    if (searchedCustomerRim != null && searchedCustomerName != null) {
      ...
      if (context != null) {
        DialogHelper.showCustomDialog(
          ...
          content: Builder(
            builder: (_) {
              const String infoKey = "requestInformation.groupBorrowers"
                  ".addedPotentialBorrower";
