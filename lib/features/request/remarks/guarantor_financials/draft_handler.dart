...
/// Handles draft save and restore operations for the
/// Guarantor Financial screen.
class GuarantorFinancialDraftHandler
  ...
  Map<String, dynamic> buildDraftData(GuarantorFinancialViewModel vm) {
    ...
    for (final guarantor in vm.state.guarantors) {
      ...
      final healthId = vm.selectedHealthFor(entityId)?.id;

      final remarks = isStrategyComment
          ? vm.remarksEditorForEntity(entityId).currentText
    ...
    for (final dynamic draft in draftList) {
      ...
      if (remarks != null) {
        ...
        if (isStrategyComment) {
          vm.remarksEditorForEntity(entityId).setText(remarks);
        } else {
