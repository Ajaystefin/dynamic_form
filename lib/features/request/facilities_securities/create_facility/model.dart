...
class CreateFacilityViewModel extends Cubit<CreateFacilityState> {
  ...
  GlobalKey dynamicFormKey = GlobalKey();
...
// Selected value for the radio (falling back to rule-based default)
  ...
  Future<void> init(bool showCreateForm, {Facility? selectedFacility}) async {
    ...
    await Future.wait([
      getDynamicForm(selectedDescriptionId),
      getcurrencyCode(),
      getCountries(),
  ...
  Future<void> getcurrencyCode() async {
    try {
      countryCodes = await repository.getcurrencyCode();
      Reference aed = countryCodes.firstWhere(
