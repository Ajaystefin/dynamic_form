...
class FacilityLimitsTable extends StatelessWidget {
  ...
  /// e.g. 11312, 11313, 11314
  final int groupId;

  /// index used to pick RimGroup from rim.groups
  final int groupIndex;

  /// Loans/PFE use autoFitWidth
  ...
  List<List<Widget>> _rows(BuildContext context, RimGroup? group, int? rimNo) {
    ...
    for (final FacilityDis facilityRow in apiDisList) {
      ...
      if (facilitySummary == null) continue;

      final String message = viewModel.tooltipMessageFor(facilitySummary);
      ...
      // Ensure selected item exists inside the filtered list
      ...
      rows.add([
        ...
        AddSublimitButton(
          ...
          selectedRim: rimNo,
          isMainLimit: false,
          proposedLimit: facilitySummary.proposedLimit,
        ...
        Align(
          ...
          child: Text(
            _kFmt.format((facilitySummary.presentLimit ?? 0).toInt()),
        ...
        CustomTooltip(
          ...
          child: CustomTextField(
            ...
            initialValue:
                _kFmt.format((facilitySummary.proposedLimit ?? 0).toInt()),
            inputFormatters: [
        ...
        Align(
          alignment: Alignment.centerRight,
          child: Text(
            _kFmt.format((facilitySummary.presentOutstanding ?? 0).toInt()),
            style: const TextStyle(color: AppColors.darkBlue),
          ),
        ),
        ...
        //tenor
        CustomTooltip(
          message: (facilitySummary.tenorValue != null ||
                  (facilitySummary.tenorUnit ?? "").isNotEmpty)
              ? "Initial Value: ${facilitySummary.tenorValue ?? ""} "
        ...
        //benchmark
        CustomTooltip(
          message: (selectedBenchmarkReference?.name ?? "").isNotEmpty
              ? "Initial Value: ${selectedBenchmarkReference?.name}"
              : "",
          child: CustomDropdown<Reference>(
            width: 125.w,
            showClearIcon: false,
            validationMessage: requireIndex ? "Index is required" : null,
            items: benchmarkItems, // filtered list
        ...
        //spread/commission
        CustomTooltip(
          message: (facilitySummary.marginValue != null ||
                  (facilitySummary.marginSign ?? "").isNotEmpty)
              ? "Initial Value: ${(facilitySummary.marginSign ?? "").trim()} "
    ...
    rows.add([
      ...
      const SizedBox(),
      Text(
        _kFmt.format((totals.totalExistingLimit ?? 0).toInt()),
        style: const TextStyle(
          fontWeight: FontWeight.bold,
        ),
      ),
      Text(
        _kFmt.format((totals.totalProposedLimit ?? 0).toInt()),
        style: const TextStyle(
          fontWeight: FontWeight.bold,
        ),
      ),
      Text(
        _kFmt.format((totals.totalCurrentOutstanding ?? 0).toInt()),
        style: const TextStyle(
          fontWeight: FontWeight.bold,
        ),
      ),
