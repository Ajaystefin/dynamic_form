...
class RiskRatingViewModel extends SafeCubit<RiskRatingState>
  ...
  /// Risk rating repository.
  late RiskRatingRepository repository;

  /// Indicates whether Credit Lens is available.
  bool isCreditLensAvailable = true;

  /// RIMs without associated entities.
  ...
  /// Initial page of the internal rating table.
  int initialInternalRatingPage = 0;

  /// Initial page of the external rating table.
  int initialExternalRatingPage = 0;

  /// Indicates whether the screen is view-only.
  bool isViewOnly = true;

  /// Indicates whether records can be deleted.
  bool isDeletable = true;

  /// Indicates whether the FI flow is active.
  bool isFiFlow = false;

  /// Controller for RIM number input.
  TextEditingController rimNoController = TextEditingController();

  /// Current risk rating data.
  late RiskRating riskRating;

  /// Updated ratings retrieved from Credit Lens.
  ...
  /// IFRS staging references.
  List<Reference> ifrsStagings = [];

  /// Number of rows displayed per page.
  int tableRow = 10;

  /// Indicates whether proposed-by-credit fields are editable.
  bool isProposedFieldEditable = false;

  /// Amendment page mode.
  PageMode? amendPagemode;

  /// Indicates whether initialization was performed by a Credit Analyst.
  bool isInitializedByCA = false;

  /// Available FI customer risk rating values.
  List<int> fiCRR = [
    1,
    2,
    3,
  ...
  // ---------------------------------------------------------------------------
  /// Indicates whether the ViewModel is initializing.
  bool isInitializing = true;

  /// Indicates whether the current page is editable.
  bool get canEdit => pageMode == PageMode.edit;

  /// Current page mode.
  PageMode pageMode = PageMode.na;

  /// Disable the Entity Id dropdown field after user selects the value
  bool isEntityFieldEnabled = true;
  ...
  Future<void> init(BuildContext context, {PageMode? amendPagemode}) async {
    ...
    isInitializedByCA =
        Globals.checkIsInitiated() && Utils.checkRole(UserRole.creditAnalyst);
    pageMode = amendPagemode ??
  ...
  /// Determines whether the current user has edit access.
  ...
  /// Determines whether rating records can be deleted by the current user.
  void checkDeleteAccess() {
    ...
    // Can delete the record for RR type application if initialted by CA
  ...
  /// Returns whether the Proposed by Credit field is editable.
  ...
  /// Returns whether the BG dropdown is editable.
  bool isBGdropdownEditable() {
    return isInitializedByCA ||
        (!isCreditLensAvailable ||
                Utils.checkRoles([
                  UserRole.relationshipOfficer,
                  UserRole.relationshipManager,
                ])) &&
  ...
  /// Retrieves the IFRS staging value for the specified customer RIM.
  ...
  /// Searches for a customer by RIM number and populates rating details.
  Future<void> searchByRim(int index, String rim) async {
    try {
      ...
      if (customerDetails.partyStatus.toString().trim() ==
        ...
        if (updatedInternalRating.any((e) => e?.isClDown ?? false)) {
        ...
        if (isRimAvailable) {
          ...
          if (!rimWithNoEntity.contains(int.tryParse(rim))) {
            ...
            AlertManager().showFailureToast("riskRating.warning".tr());
          }
        }
      }
      emit(state.copyWith(loaderStatus: LoadingStatus.loaded));
  ...
  /// Updates rating details when an entity is selected.
  Future<void> onSelectedEntities({
    ...
    final updatedInternalRating = await repository.getUpdatedRatingDetails(
      rimNo: isSearchEntity ?? false ? null : rim,
      entityId: entity,
    );
    ...
    if (showAlert) {
      ...
      isEntityFieldEnabled = false;
      emit(state.copyWith(loaderStatus: LoadingStatus.loaded));
    }
  }

  /// Adds a new row to the internal ratings table.
  ...
  Future<void> removeInternalTableRow(int? customerRiskRatingId) async {
    final matches = riskRating.internalRatings
        .where((item) => item.customerRiskRatingId == customerRiskRatingId);

    final internalRating = matches.isNotEmpty ? matches.first : null;

    if (internalRating?.customerRiskRatingId == customerRiskRatingId) {
      // if (internalRating?.rimWithNoEntity ?? false) {
    ...
    if (internalRating?.isDeletable ?? false) {
      await repository.saveRatings(
        customerRating: RiskRating.toJson(
          isFiFlow: false,
          internalRatings: [internalRating ?? InternalRating()],
    ...
    if (!riskRating.internalRatings.any(
      (item) => (item.isDeletable ?? false) && item.isDeleted != true,
    )) {
      isCreditLensAvailable = true;
    }
  ...
  /// Checks whether internal rating table has unique entries(customerRimNo + entityId).
  bool isInternalDuplicate({required int? rim, int? entityId}) {
    final Set<String> seen = <String>{};

    for (final InternalRating rating in riskRating.internalRatings) {
      final String key = "${rating.customerRimNo}|${rating.entityId}";
  ...
  /// Validates internal ratings for duplicate RIM and entity combinations
  /// before saving.
  ...
  /// Determines whether the provided map contains duplicate key-value pairs.
  ...
  /// Checks whether the specified external rating RIM already exists.
  ...
  /// Refreshes internal rating data from Credit Lens and updates
  /// the current risk rating information.
  Future<void> updateRatingsFromCL() async {
    try {
      for (final InternalRating existingRating in riskRating.internalRatings) {
        ...
        if (updatedRatings.any((e) => e?.isClDown ?? false)) {
          isCreditLensAvailable = false;
          emit(state.copyWith(loaderStatus: LoadingStatus.loaded));
          return;
        } else {
      ...
      if (updatedRiskRating.isNotEmpty) {
        for (InternalRating internalRating in riskRating.internalRatings) {
          final List<int?> entities = updatedRiskRating
              .where((e) => e?.rimNo == internalRating.customerRimNo)
              .map((e) => e?.entityId)
              .toList();
          // final List<int?> entities = updatedRiskRating
          //     .map(
          ...
          for (final UpdatedRating? updatedRating in updatedRiskRating) {
            ...
            if ((internalRating.customerRimNo == updatedRating?.rimNo) &&
              ...
              internalRating = InternalRating().fromUpdatedRatings(
                ...
                isMultipleEntity:
                    internalRating.entityId == null && entities.length > 1,
              );
            }
          }
        }
  ...
  Future<void> onSavePressed() async {
    try {
      ...
      logger.i("isProposedFieldEditable $isProposedFieldEditable");
  ...
  /// Emits a state update to refresh the internal rating UI.
