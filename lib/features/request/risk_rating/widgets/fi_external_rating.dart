...
/// FI External Rating
///
/// Displays external rating details and comments for
/// Financial Institution customers.
class FiExternalRating extends StatelessWidget {
  /// Creates an FI external rating widget.
  const FiExternalRating(this.viewModel, {super.key});
    ...
    return Column(
      ...
      children: [
        ...
        if (!viewModel.isViewOnly && viewModel.canEdit)
          AddItemButton(
            onTap: () async => viewModel.addExternalTableRow(),
            isLeftSided: true,
            child: Text("riskRating.addExternalRating".tr()),
          ),
        const Gap(),
        LabelWidget(
          label: "Bank Exposure Summary Comments",
          child: UnifiedTextEditor(
            scrollController: viewModel.externalRatingScrollController,
            characterLimit: 1000,
            height: 300,
            editorId: "externalRatingEditor",
            initialText: viewModel.riskRating.externalComments,
            controller: viewModel.externalRatingControler,
          ),
        ),
      ],
    );
  ...
  List<List<Widget>> _rows() {
    ...
    return List.generate(externalRatings.length, (i) {
      ...
      for (final Reference rating in externalRating.ratings ?? []) {
      ...
      return [
        ...
        if ((viewModel.riskRating.externalRatings ?? [])
            .any((item) => item.isDeleted ?? false))
          _buildWidget(
            child: (externalRating.isDeleted ?? false)
                ? IconButton(
