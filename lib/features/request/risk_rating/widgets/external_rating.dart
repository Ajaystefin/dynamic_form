...
///
/// Displays and manages external customer risk ratings.
class ExternalRatingTable extends StatelessWidget {
  /// Creates an external rating table.
  ...
  List<List<Widget>> _rows() {
    ...
    final List<ExternalRating> externalRatings = viewModel
            .riskRating.externalRatings
    ...
    return List.generate(externalRatings.length, (i) {
      ...
      return [
        ...
        if ((viewModel.riskRating.externalRatings ?? [])
            .any((item) => item.isDeletable ?? false))
          _buildWidget(
            child: (externalRating.isDeletable ?? false)
                ? IconButton(
