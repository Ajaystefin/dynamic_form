...
class InternalRatingTable extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    bool deleteColumn = ((viewModel.riskRating.internalRatings)
    ...
    return Column(
      ...
      children: [
        ...
        if (!viewModel.isViewOnly)
          AddItemButton(
            onTap: () async => await viewModel.addInternalTableRow(),
            isLeftSided: true,
        ...
        if (viewModel.rimWithNoEntity.isNotEmpty &&
          ...
          const Gap(),
          ...List.generate(viewModel.rimWithNoEntity.length,
  ...
  Widget _multiRim({required int rimNo}) {
    return CustomAccordion(title: "$rimNo", children: [
      SearchEntity(
        viewModel,
        rimNo: rimNo,
      ),
  ...
  List<TableColumn> _columns(bool deleteColumn) {
    return [
      ...
      TableColumn(
          width: 50.w, label: Text("riskRating.crr".tr()), isStacked: true),
      TableColumn(
          width: 90.w,
  ...
  List<List<Widget>> _rows(bool deleteColumn) {
    ...
    return List.generate(internalRatings.length, (index) {
      ...
      String? entityIdValue = internalRating.entityId != null
      ...
      return [
        ...
        _customTextField(
          ...
          onChanged: (String? customerRimNo) {
            internalRating.customerRimNo = int.tryParse(customerRimNo ?? "");
          },
        ),
        // cust name
        ...
        (internalRating.entities ?? []).length > 1
            ? CustomDropdown<int?>(
                ...
                onSelected: (value) async => await viewModel.onSelectedEntities(
                    index: index,
        ...
        if ((viewModel.riskRating.internalRatings)
            .any((item) => item.isDeletable == true))
          internalRating.isDeletable == true
              ? Center(
                  key: UniqueKey(),
                  child: IconButton(
                      onPressed: () => viewModel.removeInternalTableRow(index),
