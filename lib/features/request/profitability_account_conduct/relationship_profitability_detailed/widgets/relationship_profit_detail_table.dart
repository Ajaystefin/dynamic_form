  ...
  List<List<Widget>> _buildRows() {
    return List.generate(details?.length ?? 0, (index) {
      ...
      return [
        ...
        Text(detail.last12Months?.toStringAsFixed(2).formatNumber() ?? "",
            style: const TextStyle(color: AppColors.primary)),
        Text(detail.next12MonthsAmount?.toStringAsFixed(2).formatNumber() ?? "",
            style: const TextStyle(color: AppColors.primary)),
        Text(detail.next12MonthsProfitabilityPercent?.toStringAsFixed(2) ?? "",
            style: const TextStyle(color: AppColors.primary)),
        Text(
            detail.next12To24MonthsAmount?.toStringAsFixed(2).formatNumber() ??
                "",
            style: const TextStyle(color: AppColors.primary)),
        Text(
            detail.next12To24MonthsProfitabilityPercent?.toStringAsFixed(2) ??
                "",
            style: const TextStyle(color: AppColors.primary)),
      ];
    });
