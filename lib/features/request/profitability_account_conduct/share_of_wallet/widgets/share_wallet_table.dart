...
class ShareOfWalletTable extends StatelessWidget {
  ...
  const ShareOfWalletTable({super.key, required this.walletList});
  ...
  Widget build(BuildContext context) {
    return CustomRawTable(
      ...
      // For columns, non-stacked columns keep their widths.
      // For stacked columns, use the computed 100.w.
      columns: [
        TableColumn(
          label: Text(
      ...
      rows: walletList
          .map(
            (wallet) => [
              ...
              Text(
                wallet.facilitiesWithAllBanksLimitsA.toStringAsFixed(2).formatNumber(),
                style: const TextStyle(color: AppColors.darkBlue),
              ),
              Text(
                wallet.facilitiesWithAllBanksOutstandingC.toStringAsFixed(2).formatNumber(),
                style: const TextStyle(color: AppColors.darkBlue),
              ),
              Text(
