...
class RelationshipUtilizationViewModel
  ...
  TextEditingController clientCtrlAt(int i) {
    if (!controllersReady) return _emptyController;
    if (i < 0 || i >= _clientTurnoverCtrls.length) return _emptyController;
    return _clientTurnoverCtrls[i];
  }

  TextEditingController cbdCtrlAt(int i) {
    if (!controllersReady) return _emptyController;
    if (i < 0 || i >= _turnoverInCbdCuaCtrls.length) return _emptyController;
    return _turnoverInCbdCuaCtrls[i];
  }

  TextEditingController pctCtrlAt(int i) {
    if (!controllersReady) return _emptyController;
    if (i < 0 || i >= _throughputPctCtrls.length) return _emptyController;
  ...
  void _recalcPercentage(int index) {
    ...
    if (index < 0 || index >= relationshipUtilizationData.length) return;
    ...
    if (client < turnoverCbd) {
      ...
      if (!_isShowingTurnoverError) {
        ...
        // Reset after a small delay (so toast can finish)
  ...
  String _sanitize15_6(String input) {
    ...
    if (out.isEmpty) return ""; // avoid just '-'
