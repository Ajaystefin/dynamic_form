...
/// Desktop view for the Income Summary screen.
class ViewDesktop extends StatelessWidget {
  /// Creates a desktop view for the Income Summary screen.
  ...
  /// Builds the income summary view content.
  Widget buildView(
    BuildContext context,
    IncomeSummaryState state,
    IncomeSummaryViewModel viewModel,
      ...
      child: Form(
        ...
        child: Column(
          ...
          children: [
            ...
            if (viewModel.incomeSummaryList!.isEmpty)
              Center(
                child: Text("common.emptyState".tr()),
              )
            else
            ...
            // view_desktop.dart — inside buildView(...)
            LabelWidget(
              label:
                  "profitabilityAccountConduct.incomeSummary.rmComments".tr(),
              labelStyle: const TextStyle(fontWeight: FontWeight.bold),
              child: CustomTextArea(
                // Prefer controller over initialValue for dynamic updates
                controller: viewModel.rmCommentsController,
                semanticLabel:
                    "profitabilityAccountConduct.incomeSummary.rmComments".tr(),
                width: double.infinity,
                maxLength: 2000,
                // Keep onSaved to mirror into VM for final save
                onSaved: (String? value) {
                  viewModel.rmComments = (value?.trim().isNotEmpty ?? false)
                      ? value!.trim()
                      : null;
                },
              ),
