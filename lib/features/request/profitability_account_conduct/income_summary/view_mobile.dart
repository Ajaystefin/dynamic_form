...
class ViewMobile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    IncomeSummaryViewModel viewModel = context.read<IncomeSummaryViewModel>();
  ...
  Widget _body(BuildContext context, IncomeSummaryState state,
  ...
  Widget buildView(BuildContext context, IncomeSummaryState state,
      IncomeSummaryViewModel viewModel) {
          ...
          children: [
            ...
            CustomRawTable(
              ...
              columns: [
                ...
                TableColumn(
                    label: Text(
                        'profitabilityAccountConduct.incomeSummary.lastMonths'
                            .tr())),
                TableColumn(
                    label: Text(
                        'profitabilityAccountConduct.incomeSummary.nextMonths'
                            .tr())),
                TableColumn(
                    label: Text(
              ...
              rows: viewModel.incomeSummaryList!
                  .map((income) => [
                        Center(
                            child: Text('${income.rimNo}',
                                style:
                                    const TextStyle(color: AppColors.primary))),
                        Center(
                            child: Text(income.custName ?? "",
                                style:
                                    const TextStyle(color: AppColors.primary))),
                        Center(
                            child: Text(
                                income.lastYearAmount!.toStringAsFixed(2),
                                style:
                                    const TextStyle(color: AppColors.primary))),
                        Center(
                            child: Text(
                                income.nextYearAmount!.toStringAsFixed(2),
                                style:
                                    const TextStyle(color: AppColors.primary))),
                        Center(
                            child: Text(
                                income.nextYear2Amount!.toStringAsFixed(2),
                                style:
                                    const TextStyle(color: AppColors.primary)))
                      ])
                  .toList(),
            ...
            Row(
              ...
              children: [
                ...
                CustomButton(
                  semanticLabel:
                      "profitabilityAccountConduct.incomeSummary.saveAndContinue"
                          .tr(),
                  label:
                      "profitabilityAccountConduct.incomeSummary.saveAndContinue"
                          .tr(),
