...
class ManagementCommentsViewModel extends SafeCubit<ManagementCommentsState>
  ...
  Future<void> init(context) async {
    try {
      ...
      debugPrint("hasAccess : $route $hasAccess");
  ...
  Future<void> onSavePress({
    ...
    try {
      ...
      if (formKey.currentState?.validate() ?? false) {
        ...
        // debugPrint(
        //     "Save Comment List : ${comments.map((ele) =>
        // ele.toStrategyJson().toString())}");
        ...
        if (context.mounted) {
          await context.read<ManagementCommentsViewModel>().init(context);
        }
      }
    } catch (e) {
