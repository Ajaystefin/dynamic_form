...
class GroupPositionViewModel extends SafeCubit<GroupPositionState>
  ...
  GroupPositionViewModel()
      : super(const GroupPositionState(loaderStatus: LoadingStatus.loading));
  ...
  /// Indicates whether the current user has edit access for group position.
  bool isEdit =
  ...
  /// Customer position groups displayed in the group position screen.
  List<CustomerPosition> groups = [];

  /// Group position data returned from the approval repository.
  ...
  /// Total present exposure value.
  double totalPresentExposure = 0;

  /// Indicates whether the group position screen is read-only.
...
// Use the correct module bucket → adjust if you have a specific constant
  ...
  /// Form key used to uniquely identify the group position draft.
  @override
  String get draftFormKey => Routes.groupPosition;
// This must be the exact route string used for this screen

  /// Draft handler used to build and apply group position draft data.
  @override
  ...
  /// Gets group position details and transforms them for display.
