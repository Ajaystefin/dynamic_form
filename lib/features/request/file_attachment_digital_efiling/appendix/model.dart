...
class AppendixViewModel extends SafeCubit<AppendixState>
  ...
  /// group corporate structure editor Id
  final String groupCorporateStructureEditorId = const Uuid().v4();

  /// List of UnifiedEditorController
  ...
  /// Show Corporate Section flag
  bool showCorporateSection = false;

  /// Show Financial Section flag
  bool showFinancialSection = true;

  /// Show Financial CF Section flag
  ...
  /// Show Rim Not Available Message flag
  bool showRimNotAvailableMessage = false;

  /// is Fi Rim Unavailable flag
  bool get isFiRimUnavailable =>
      isFI &&
      selectedSectionType == ServerConstants.bigBank &&
      rimNumbers.isEmpty;
  ...
  /// is Appendix Read Only flag
  bool get isAppendixReadOnly => !canEdit;
    ...
    try {
      ...
      _toastSafe(
        () => AlertManager().showSuccessToast(
          "eDigitalFilingFileAttachments.appendix.fetchedRecords"
              .tr(namedArgs: {"count": fiServerRows.length.toString()}),
        ),
      );
    } on Object catch (e) {
  ...
  Future<void> autoLoadFiAppendixXlsx() async {
    ...
    try {
      ...
      // Apply filter based on selectedRimNumber
      _filterExcelRows();

      refresh();
    } on Object catch (e, st) {
      logger
        ..i("autoLoadFiAppendixXlsx error: $e")
        ..i("Stack trace", stackTrace: st);
    } finally {
      _emitSafe(state.copyWith(loaderStatus: LoadingStatus.loaded));
    }
  }
  ...
  Future<void> getApplicationDetails(String? applicationId) async {
    try {
      ...
      _rebuildRimNumbersFromApplicationDetails();
    } on Object catch (e, st) {
      logger.i("getApplicationDetails error: $e\n$st");
    } finally {
      _emitSafe(state.copyWith(loaderStatus: LoadingStatus.loaded));
    }
  }

  void _rebuildRimNumbersFromApplicationDetails() {
    ...
    if (selectedSectionType == ServerConstants.country) {
      targetType = CustomerType.country;
    } else if (selectedSectionType == ServerConstants.bigBank) {
      targetType = CustomerType.belowInvestmentGradeBanks;
    } else {
      targetType = null;
    }

    if (borrowers.isEmpty || targetType == null) {
      rimNumbers.clear();
      selectedRimNumber = null;
      _filterExcelRows();
      refresh();
    ...
    final List<String> rims = borrowers
        ...
        .map((b) => (b.customerRimNo?.toString() ?? "").trim())
        .where((rim) => rim.isNotEmpty)
        .toSet()
        .toList()
      ..sort();
    ...
    if (selectedRimNumber != null && rimNumbers.contains(selectedRimNumber)) {
      // keep existing
    } else {
  ...
  String fileNamesToText(Object? fileNames) {
    if (fileNames == null) {
      return "-";
    }
  ...
  Future<void> init(BuildContext context) async {
    ...
    unawaited(autoLoadFiAppendixXlsx());
  ...
  List<String> get countryRatingOptions {
    return sAndP
        .map(
          (Reference rating) => (rating.name ?? rating.reference1 ?? "").trim(),
        )
  ...
  Future<void> onUploadFiExcel(BuildContext context) async {
    ...
    try {
      ...
      logger.i("Excel Upload failed $e");
  ...
  Future<void> deleteFetchedStrengthAt(int index) async {
    ...
    try {
      ...
      appendix.strengths.removeAt(index);
      if (fetchedCount > 0) {
        _fetchedStrengthCounts[toDelete] = fetchedCount - 1;
      }
  ...
  Future<void> deleteFetchedThreatAt(int index) async {
    ...
    try {
      ...
      appendix.threats.removeAt(index);
      if (fetchedCount > 0) {
        _fetchedThreatCounts[toDelete] = fetchedCount - 1;
      }
  ...
  Future<void> fetchAppendixBusinessSegment() async {
    ...
    try {
      ...
      logger.i("fetchAppendixBusinessSegment failed: $e");
    } finally {
      _emitSafe(state.copyWith(loaderStatus: LoadingStatus.loaded));
    }
  }
  ...
  Future<void> deleteFetchedBankItemAt(int index) async {
    ...
    if (index < 0 || index >= bankFinancialFileIds.length) {
      return;
    }
  ...
  Future<void> deleteFetchedCountryItemAt(CountryImage slot, int index) async {
    ...
    if (!indexInIds) {
      logger.i("No server id at index=$index; removing local file only.");
      if (indexInFiles) {
  ...
  void onAddAppendix() {
    if (!_requireAppendixEditPermission()) {
      return;
    }
  ...
  void updateSelectedSectionType(String value) {
    ...
    showRimNotAvailableMessage = isFI &&
  ...
  DateTime get setSelectedDate => selectedDate!;

  set setSelectedDate(DateTime value) {
    selectedDate = value;
  }

  String? get countryName => appendix.countryName;
  ...
  Future<void> removeAnyFile(PlatformFile target) async {
    ...
    // Local temp lists
    if (removeLocalByTarget(fiKeyFinancialFiguresExcelFiles)) {
      return;
    }
    if (removeLocalByTarget(fiKeyFinancialFiguresImageFiles)) {
      return;
    }
    if (removeLocalByTarget(bankFinancialFiles)) {
      return;
    }

    for (final MapEntry<CountryImage, List<PlatformFile>> entry
        in countryFiles.entries) {
      if (removeLocalByTarget(entry.value)) {
        return;
      }
    }

    if (removeLocalByTarget(selectedFiles)) {
      return;
    }
  ...
  Future<void> deleteFiExtractXlsxById(int appendixXlsxId) async {
    try {
      ...
      if (idx != -1) {
        if (idx < fiExtractXlsxFiles.length) {
          fiExtractXlsxFiles.removeAt(idx);
        }
  ...
  List<PlatformFile> applySectionFilter(List<PlatformFile> input) {
    ...
    if (isBankSection) {
      return filterBankFinancial(input);
    }
    if (isCountrySection) {
      return filterCountryImages(input);
    }
    return input;
  }
  ...
  bool _validateRimNoSelection() {
    final bool requiresRimValidation =
        selectedSectionType == ServerConstants.bigBank ||
            selectedSectionType == ServerConstants.country;

    if (!requiresRimValidation) {
      return true;
    }
    ...
    final selected = selectedRimNumber?.trim();

    if (selected == null ||
        selected.isEmpty ||
        !rimNumbers.contains(selected)) {
      _toastSafe(
        () => AlertManager().showFailureToast(
          "eDigitalFilingFileAttachments.appendix.selectRimNo".tr(),
        ),
      );
      return false;
    }
  ...
  Future<void> saveAllComments({required bool isContinue}) async {
    ...
    try {
      ...
      if (isContinue) {
        router.go(Routes.proposedFacilities);
      }
    } on Object catch (e) {
      _toastSafe(() => AlertManager().showFailureToast(e.toString()));
    } finally {
  ...
  Future<void> getComments() async {
    ...
    try {
      ...
      appendix.groupCorporateStructure = commentList.first.strategyComment!;
      emit(state.copyWith(loaderStatus: LoadingStatus.loaded));
    } on Object catch (e, st) {
