...
/// UploadedFileDetails stateless widget
class UploadedFileDetails extends StatefulWidget {
  /// Creates [UploadedFileDetails] instance
...
class _UploadedFileDetailsState extends State<UploadedFileDetails> {
  ...
  Widget _buildView(BuildContext context, FileAttachmentViewModel viewModel) {
    ...
    rows = documentList.asMap().entries.map((entry) {
      ...
      final fileNames = document.files?.isNotEmpty ?? false
      ...
      return [
        ...
        Text(
          document.documentName?.isNotEmpty ?? false
              ? document.documentName!
              : "N/A",
        ),
        if (document.deleteLoader == LoadingStatus.loading)
          const Padding(
            padding: EdgeInsets.all(8),
            child: SizedBox(
              width: 14,
