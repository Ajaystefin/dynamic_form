...
class SelectedFilesList extends StatelessWidget {
  ...
  List<List<Widget>> _rows() {
    return viewModel.selectedDocuments.asMap().entries.map((p) {
      ...
      final fileNames = doc.files?.isNotEmpty ?? false
      ...
      return [
        ...
        Text(
          (doc.groupRim != null && doc.groupRim != 0)
              ? doc.groupRim.toString()
              : "N/A",
        ),
        CustomTooltip(
          message: doc.companyRim ?? "N/A",
          child: Text(doc.companyRim ?? "N/A"),
        ),
        Text(doc.subType?.name ?? "N/A"),
        Text(doc.subSubType?.name ?? "N/A"),
        Text(doc.subSubSubType?.name ?? "N/A"),
        Text(
          doc.applicationId?.isNotEmpty ?? false ? doc.applicationId! : "N/A",
        ),
        Text(
          doc.date != null ? DateFormat("dd-MM-yyyy").format(doc.date!) : "N/A",
        ),
        Text(doc.language?.name ?? "N/A"),
        Text(doc.documentName?.isNotEmpty ?? false ? doc.documentName! : "N/A"),
        CustomTooltip(
          message: fileNames,
          child: IconButton(
            onPressed: () {
