...
import "package:wcas_frontend/features/request/file_attachment_digital_efiling/file_attachment/widgets/legacy_file_details.dart";
  ...
  Widget _buildView(
    ...
    return Column(
      ...
      children: [
        ...
        if (viewModel.state.showUploadButton ?? false)
          Row(
            ...
            children: [
              CustomButton(
                label:
                    "eDigitalFilingFileAttachments.fileAttachments.uploadFiles"
                        .tr(),
                    ...
                    children: [
                      ...
                      if (viewModel.isConstitutionalDocumentsSelected() ||
                        ...
                        DocumentNameField(
                          ...
                          documentTypeId: viewModel.selectedDocumentType?.id,
                          subSubTypeId: viewModel.selectedSubSubType?.id,
                        ),
                      ],
        ...
        const Gap(),
        if (viewModel.legacyFiles.isNotEmpty)
          CustomSectionHeader(
            title: "eDigitalFilingFileAttachments.fileAttachments.legacyFiles"
                .tr(),
          ),
        if (viewModel.legacyFiles.isNotEmpty) const Gap(),
        if (viewModel.legacyFiles.isNotEmpty)
          LegacyFileDetails(
            viewModel: viewModel,
          ),
        const Gap(),
        CustomSectionHeader(
          title: "eDigitalFilingFileAttachments.fileAttachments.archivedFiles"
              .tr(),
        ),
        const Gap(),
        switch (state.digitalFilesStatus) {
          LoadingStatus.loading => const Center(
              child: CircularProgressIndicator(),
            ),
          _ => rimListAccordian(viewModel),
        },
        const Gap(),
