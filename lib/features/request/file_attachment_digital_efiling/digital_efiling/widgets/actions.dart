  ...
  return Row(
    ...
    children: [
      if (viewModel.selectedDocumentIds.isNotEmpty)
        CustomButton(
            label:
                "eDigitalFilingFileAttachments.fileAttachments.download".tr(),
            onPressed: () async {
              viewModel.downloadDocumentsZip();
            }),
      const Gap(
      ...
      if (viewModel.selectedDocumentIds.isNotEmpty)
        CustomButton(
            label: "eDigitalFilingFileAttachments.fileAttachments.mergeDownload"
                .tr(),
            onPressed: () async {
              viewModel.mergeDownloadDocument();
            }),
      const Gap(
        direction: Axis.horizontal,
      ),
      if (viewModel.buttonVisibilityStatus[DigitaleFileFields.uploadDocument]
        ...
        CustomButton(
            ...
            onPressed: () async {
              viewModel.openUploadDialog(context);
            }),
    ],
  );
}
