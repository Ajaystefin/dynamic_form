...
class SecurityField extends StatelessWidget {
  ...
  SecurityField({super.key, required this.viewModel});
  ...
  Widget build(BuildContext context) {
    ...
    return LabelWidget(
        label: 'groupInformation.facilitiesWithOtherBanks.security'.tr(),
        isRequired: (viewModel.showCurrentFiCreditRisk) ? false : true,
        showLabel: true,
        child: CustomMultiSelectDropdown<Reference>(
          key: ValueKey(
            viewModel.currentFacilityItems.securityWith?.length,
          ),
          semanticLabel:
          ...
          items: viewModel.securityOptions
              .where((e) => (e.name ?? "").trim().isNotEmpty)
          ...
          dropdownBuilder: (context, data) {
            ...
            return multiSelectDropDownBuilderWidget(
              ...
              itemBuilder: (index) {
                ...
                // Resolve name by id, fallback to ref.name (keeps your visual logic)
          ...
          onSelected: (selected) {
            // keep your delegate
            viewModel.onSecurityTypeSelected(selected);
          ...
          selectedItems: (() {
            final selected = viewModel.currentFacilityItems.securityWith ??
                const <Reference>[];
            if (selected.isEmpty) return const <Reference>[];

            final selectedIds =
                selected.map((r) => r.id).whereType<int>().toSet();
            // Rebuild selectedItems from the same 'items' source to align instances
            return viewModel.securityOptions
