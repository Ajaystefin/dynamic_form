...
/// View model for the Add CBRB dialog.
class AddCbrbDialogViewModel extends SafeCubit<AddCbrbDialogState> {
  /// Creates an [AddCbrbDialogViewModel].
  ...
  /// Indicates whether a new CBRB record is being added.
  bool isAddNew = true;

  /// Initial CBRB record used while editing.
  CBRB? initialCbrbs;
  ...
  Future<void> init(
    ...
    if (initialCbrb != null) {
      ...
      final int rimNo = initialCbrb.rimNo ?? 0;
      ...
      emit(
        state.copyWith(
          ...
          customerName:
              initialCbrb.customerName ?? selectedCustomer?.displayName,
        ),
      );
    }
  ...
  /// Retrieves the required reference data.
