...
class ConditionEditDialogViewModel extends Cubit<ConditionEditDialogState> {
  ...
  bool get isViewOnlyMode => pageMode == PageMode.view;

  /// Initializes the view model.
  ///
  /// Sets up the repository, loads reference data, and pre-fills condition details if available.
  ///
  /// [context] - The BuildContext, if needed for additional setup.
  /// [condition] - The existing covenant condition, if provided.
  Future<void> init(context, CovenantCondition? condition) async {
  ...
  void getModifyData() {
    try {
      ...
      if (conditionData?.customerName != null) {
        selectedCustomer = Customer(customerName: conditionData?.customerName);
      }
      ...
      if (conditionData?.isStandard ?? false) {
        referenceData[ReferenceDataKeys.conditionStandard]?.map(
          (element) {
            if (element.id == ServerConstants.conditionStandardId) {
              selectedDescriptionType = element;
            }
          },
        ).toList();
      } else {
        referenceData[ReferenceDataKeys.conditionStandard]?.map(
          (element) {
            if (element.id == ServerConstants.conditionCustomId) {
              selectedDescriptionType = element;
            }
          },
        ).toList();
  ...
  ///
  /// Validates the form and then attempts to save the condition data via the repository.
  ...
  Future<bool> onSavePress() async {
    try {
      ...
      String? response = await repository.saveConditionDetails(conditionData);
      AlertManager().showSuccessToast(
        response.toString(),
      );
      router.pop();
 ...
 //parse target date field
  ...
  void onGeneralFieldChanged(
      Reference selectedValue, BuildContext context) async {
    ...
    if (isSpecificSelected()) {
      final data = await DialogHelper.showCustomDialog(
        ...
        content: SelectFacilitiesDialogView(selectedFacility: facilityList),
        context: context,
      );
