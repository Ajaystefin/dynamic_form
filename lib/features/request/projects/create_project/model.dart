...
class CreateProjectViewModel extends SafeCubit<CreateProjectState>
  ...
  /// Creates a create project view model.
  CreateProjectViewModel()
      : super(CreateProjectState(loaderStatus: LoadingStatus.loading));

  /// Form key for create project form.
  ...
  /// Indicates whether this is create project mode.
  ...
  Future<void> init(
    ...
    try {
      ...
      logger..i("------")
      ..i(Globals.request?.applicationRefNo);
...
// --------------------------------------------------
  ...
  /// Syncs model values from controllers.
  ...
  /// Syncs controller values from model.
  ...
  Future<void> onSave(BuildContext context, {required bool isValidate}) async {
  ...
  Future<void> onCreate(
    ...
    try {
      ...
      if (message != null &&
          message != "project.createNewProject.projectSaved".tr()) {
  ...
  /// Handles link contract button press.
  ...
  /// Handles contract code press in table.
  ...
  /// Generates project exposure summary.
  ...
  /// Handles back to request status press.
  ...
  String? projectCodeValidator(String? value) {
    final v = (value ?? "").trim();
    if (v.isEmpty) {
      return "project.code.required".tr();
    }
    if (v.length != 16) {
      return "project.code.lengthInvalid".tr(args: ["16"]);
    }
    final regex = RegExp(r"^\d{4}(0[1-9]|1[0-2])PROJ\d{6}$");
    if (!regex.hasMatch(v)) {
      return "project.code.formatInvalid".tr();
    }
    return null;
  }

  /// Shows project success dialog.
...
class ProjectCodeGenerator {
  ...
  /// Checks whether project code is valid.
