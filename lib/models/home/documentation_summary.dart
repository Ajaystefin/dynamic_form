...
class DocumentationStage {
  /// Creates a [DocumentationStage] instance with a total count and categories.
  DocumentationStage({
...
class DocumentationSummary {
  /// Creates a [DocumentationSummary] instance with the provided stages.
  ...
  /// Converts this [DocumentationSummary] instance into a JSON map.
  Map<String, dynamic> toJson() {
...
/// - List<> (flat rows)  --> uses fromFlatList
/// - Map< (object) --> uses fromJson
/// - Anything else -> throws a descriptive error
class DocumentationSummaryParser {
  /// Parses the given data into a [DocumentationSummary].
  static DocumentationSummary parse(Object? data) {
    if (data is List) {
