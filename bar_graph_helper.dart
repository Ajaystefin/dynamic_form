...
/// A helper class containing data transformation and grouping logic for the BarGraph.
class BarGraphHelper {
  /// Builds a global category order based on occurrences across all stages.
  /// Prioritizes the counts of the [stageForPriority] if provided, then falls back to alphabetical.
  ...
  /// Returns all stage keys in a stable order. Applies specific top-level organization
  /// for CCU roles and Credit Control summary type.
  static List<String> stageDisplayOrder(
      DocumentationSummary? summary, HomeViewModel viewModel) {
    if (summary == null) return const [];
  ...
  /// Capitalizes the first letter of each word in a stage key string.
  static String prettifyStage(String stageKey) {
    if (stageKey.trim().isEmpty) return stageKey;
    return stageKey
        .split(' ')
