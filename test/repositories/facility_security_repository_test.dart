...
AppResponse _successResponse(dynamic body, {String message = "OK"}) =>
    AppResponse(
...
void main() {
  ...
  group("saveSecurityDetails", () {
    ...
    test("skips transform when dynamicFormDocument is null", () async {
      ...
      expect(
        await repository.saveSecurityDetails(
          Security(dynamicFormDocument: null),
          [],
          "SEC004",
        ),
        isNotNull,
    ...
    test("sorts rows by index", () async {
      final doc = {"g.a@1": "1a", "g.b@1": "1b", "g.a@0": "0a", "g.b@0": "0b"};
      ...
      await repository.saveSecurityDetails(
        Security(dynamicFormDocument: doc),
        [
          _section([
            _gridField(key: "g", columns: ["a", "b"]),
          ]),
        ],
        "X",
      );
  ...
  group("deleteSecurityDetails", () {
    ...
    test("throws on error", () {
      ...
      expect(
        ...
        throwsA(contains("Delete failed")),
      );
    });
  });
  ...
  group("getProjectList", () {
    ...
    test("returns empty ProjectListResponse on error (toast suppressed)",
      ...
      try {
        ...
        expect(result, const ProjectListResponse([]));
      } catch (e) {
  ...
  group("saveFacilityDetailsNew", () {
    ...
    test("skips transform when additionalDetails is null", () async {
      ...
      await repository.saveFacilityDetailsNew(
        facilityDetails: FacilityDetails(additionalDetails: null),
      );
    });
  ...
  group("getBorrowersMap", () {
    test("throws on error", () {
      ...
      expect(() => repository.getBorrowersMap(), throwsA(contains("MapFail")));
    });

    test("throws StateError when response body is null", () {
      when(() => mockApiManager.post(APIEndpoints.getBorrowersMap, any()))
          .thenAnswer(
        (_) async => AppResponse(
          status: ResponseStatus.success,
