  ...
  group("ApprovalRepository Integration Tests", () {
    ...
    group("getGroupPositionDetails - Success Scenarios", () {
      test("should successfully get group position details", () async {
        ...
        expect(result.proposedPosition, hasLength(2));
        // expect(result.presentPosition, null);

        // Verify proposed position data
        expect(
          result.proposedPosition![0].customerName,
    ...
    group("getOutputForms - Success Scenarios", () {
      test("should successfully get output forms", () async {
        ...
        mockReferenceDataService.setMockReferenceData = {
          ReferenceDataKeys.caOutputForms: [
            Reference(id: 101, name: "Form A"),
            Reference(id: 102, name: "Form B"),
            Reference(id: 103, name: "Form C"),
          ],
        };
      ...
      test("should handle forms with null names", () async {
        ...
        mockReferenceDataService.setMockReferenceData = {
          ReferenceDataKeys.caOutputForms: [
            Reference(id: 101), // null name default
            Reference(id: 102, name: "Valid Form"),
          ],
        };
    ...
    //     // Act & Assert
    //     expect(
    //       () async => approvalRepository.getOutputForms(),
    ...
    group("downloadOutputForms", () {
      test("should open file when isDownload is false", () async {
        final base64Data = base64Encode(Uint8List.fromList([1, 2, 3]));
        ...
        await approvalRepository.downloadOutputForms(
          forms,
          "pdf",
          isDownload: false,
        );
      ...
      test("should download selected forms successfully", () async {
        final base64Data = base64Encode(Uint8List.fromList([1, 2, 3]));
    ...
    group("Edge Cases and Integration", () {
      ...
      test("should handle large datasets efficiently", () async {
        ...
        // Assert
        expect(result.proposedPosition, hasLength(1000));
    ...
    //       throwsExceptionWithMessage("Timeout"),
    //     );
    //   });
    // });

    // group("getFilteredUsersByrole", () {
    //   test("success | responseData List -> returns users with mapped role",
    //       () async {
    //     Globals.user = User(
    //       segments: ["Segment1"],
    //       regions: ["Region1"],
    ...
    //     final list = await approvalRepository.getFilteredUsersByrole(["RO"]);

    //     expect(list.length, 1);
    //     expect(list.first.id, "123");
    //     expect(list.first.currentRole?.name, "RO");
    //     expect(list.first.currentRole?.roleId, 125);
    //     expect(list.first.currentRole?.bpmRole, "RO-WCAS");
    //     expect(
    ...
    group("fetchReference", () {
      test(" should populate Globals correctly", () async {
        // Arrange
        mockReferenceDataService.setMockReferenceData = {
          ReferenceDataKeys.requestStatus: [
            Reference(id: 1, name: "Approved"),
          ],
          ReferenceDataKeys.userActionType: [
            Reference(id: 10, name: "Submit"),
          ],
          ReferenceDataKeys.roleType: [
            Reference(id: 100, reference1: "ADMIN", reference3: "BPM_ADMIN"),
          ],
          ReferenceDataKeys.folTypes: [
            Reference(id: 200, name: "FOL"),
          ],
        ...
        expect(Globals.superBpmRolesId, [
          {"BPM_ADMIN": 100},
        ]);
        expect(Globals.superBpmRoles, ["BPM_ADMIN"]);
        expect(Globals.superRolesId, [
          {"ADMIN": 100},
        ]);
        ...
        expect(Globals.reasonForDecline, contains("Invalid Data"));
      });
    });

    // group("validateRSAToken", () {
    //   test("return true for success", () async {
    //     mockAPIManager.setMockResponse(
    //       AppResponse(
    //         message: "ok",
    //         body: {
    ...
    //       ),
    //     );
    //     final result = await approvalRepository.validateRSAToken("456987234");
    //     expect(result, true);
    //   });
    // });
    ...
    group("saveReviewComments", () {
      ...
      test("should throw error when response status is error", () async {
        ...
        expect(
          ...
          throwsExceptionWithMessage("Internal Server Error"),
        );
      });
    });
