...
class MockAPIManager implements APIManager {
  ...
  Future<AppResponse> post(
    ...
    callLog.add(CallLogEntry(method: "post", endpoint: endPoint, body: body));
  ...
  Future<AppResponse> downloadFile(
    ...
    callLog.add(
      CallLogEntry(method: "downloadFile", endpoint: endPoint, body: body),
    );
...
void main() {
  ...
  test("should successfully extract appendix XLSX and return statusDescription",
      () async {
    // Arrange
    final bytes = Uint8List.fromList([1, 2, 3]);
    const fileName = "appendix.xlsx";

    mockAPIManager.setMockResponse(
      successResp(base: {"statusDescription": "File extracted successfully"}),
    );
    ...
    // Assert
    expect(result, equals("File extracted successfully"));
    expect(mockAPIManager.callLog, hasLength(1));
    final lastCall = mockAPIManager.callLog.last;
    expect(lastCall.method, "uploadFile");
    expect(lastCall.endpoint, APIEndpoints.extractAppendixXlsx);
    expect(lastCall.fileNameOverride, fileName);
    expect(lastCall.fileBytes, bytes);
    expect(lastCall.fieldName, "file");
    expect(lastCall.additionalData, containsPair("rimNo", "RIM123"));
    expect(lastCall.additionalData, containsPair("userID", "USER999"));
    expect(lastCall.additionalData, containsPair("appRefNo", "APP789"));
  });
  ...
  test("should return [] when responseData is null (fetchFiAppendixXlsx)",
      () async {
    mockAPIManager.setMockResponse(
      successResp(base: {"statusDescription": "OK"}, responseData: null),
    );
  ...
  test("should throw message when status != success (getAppendixImageBase64)",
      () async {
    mockAPIManager.setMockResponse(
      errorResp(message: "Not OK", statusDescription: null),
    );
  ...
  test(
    ...
    mockAPIManager.setMockResponses(
      post: successResp(base: {"statusDescription": "OK"}, responseData: null),
      downloadFile: AppResponse(
        status: ResponseStatus.success,
        code: 200,
        message: "OK",
  ...
  test("should throw when status != success (fetchAppendixImageItems)",
      () async {
    mockAPIManager.setMockResponse(
      errorResp(message: "Fail", statusDescription: null),
    );
  ...
  // ========= saveGroupCorporateStructureComment =========
  test(
      "should throw normalized on failure (saveGroupCorporateStructureComment)",
      () async {
    mockAPIManager.setMockResponse(
      errorResp(message: "msg", statusDescription: "Cannot save"),
    );
    ...
    const payload = GroupCorporateStructureCommentPayload(
      ...
      createdBy: "tester",
      updatedBy: "tester",
      comments: "Sample test comment",
      commentType: "Group Corporate Structure",
      appendixRemarkId: 0, // or null if your model allows
      // createdDate: '2024-01-01T00:00:00Z',
      // updatedDate: '2024-01-01T00:00:00Z',
    );
    expect(
      () => appendixRepository.saveGroupCorporateStructureComment(payload),
      throwsA("Cannot save"),
    );
  });
  ...
  test(
      "should return statusDescription on success "
      "(saveGroupCorporateStructureComment)", () async {
    mockAPIManager.setMockResponse(
      successResp(base: {"statusDescription": "Saved Comment"}),
    );
    ...
    final msg =
        await appendixRepository.saveGroupCorporateStructureComment(payload);
    expect(msg, "Saved Comment");
    expect(mockAPIManager.callLog, hasLength(1));
    expect(
      mockAPIManager.callLog.first.endpoint,
