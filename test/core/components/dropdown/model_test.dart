...
void main() {
  group("CustomDropdownItem", () {
    test("should create instance with required value", () {
      ...
      expect(item.label, equals(testValue));
  ...
  group("groupItemsByTitle", () {
    ...
    test("should handle items with null titles", () {
      final items = [
        ...
        CustomDropdownItem(value: "item3", title: "Group A"),
        CustomDropdownItem(value: "item4"),
      ];
    ...
    test("should handle all items with null title", () {
      final items = [
        ...
        CustomDropdownItem(value: "item2"),
        CustomDropdownItem(value: "item3"),
      ];
  ...
  group("handleKeyEvent (simple unit tests)", () {
    KeyDownEvent key(LogicalKeyboardKey key) {
      return KeyDownEvent(
        logicalKey: key,
    ...
    test("enter key does not crash and calls unfocus", () {
    ...
    test("arrowDown does nothing if not focused", () {
      final focusNode = FocusNode();

      handleKeyEvent(key(LogicalKeyboardKey.arrowDown), focusNode);

      expect(focusNode.hasFocus, false);
    });

    test("arrowUp does nothing if not focused", () {
      final focusNode = FocusNode();

      handleKeyEvent(key(LogicalKeyboardKey.arrowUp), focusNode);

      expect(focusNode.hasFocus, false);
    });

    test("non KeyDownEvent is ignored", () {
      final focusNode = FocusNode();
      ...
      handleKeyEvent(event, focusNode);
