...
void main() {
  ...
  group("CurrencyDropdown", () {
    ...
    testWidgets("does not show AED equivalent row when aedEquivalent is 0",
      ...
      await tester.pumpWidget(
        buildCurrencyDropdown(
          options: [Option(key: "AED", pairValue: "AED"), usdOption],
          initialOption: usdOption,
          initialAedEquivalent: 0, // exactly 0, should NOT initialize
        ),
      );
      await tester.pumpAndSettle();
    ...
    testWidgets(
        "didUpdateWidget: clears AED equivalent when"
        " set to null and no controller", (WidgetTester tester) async {
      final Option aedOption = Option(key: "AED", pairValue: "AED");
  ...
  group("DynamicFormCurrencyDropdownTextfield", () {
    ...
    testWidgets("does not crash when document has non-map value for field key",
      ...
      // Document has a string instead of a map — extractFromDocument returns
      // null
      await tester.pumpWidget(
        buildDynamicWidget(
          fieldData: field,
  ...
  group("CurrencyDropdown advanced didUpdateWidget paths", () {
    testWidgets(
        "switching currency "
        "from AED to non-AED "
        "triggers currency change path and onChanged",
        (WidgetTester tester) async {
      final Option aedOption = Option(key: "AED", pairValue: "AED");
      final Option usdOption = Option(key: "USD", pairValue: "USD");
      final TextEditingController controller =
      ...
      // Switch to USD — triggers async addPostFrameCallback for exchange rate
      // fetch
      ...
      // Pump past the 300ms rate debounce + MockInterceptor 500ms delay.
      // pumpAndSettle cannot be used once a fetch is in flight: the AED field
      // shows a loader, and an animating indicator never settles.
      await tester.pump(const Duration(milliseconds: 400));
      await tester.pump(const Duration(seconds: 1));
      await tester.pump();

      // onChanged should have been called with USD currency from
      // getCurrencyRates
    ...
    testWidgets(
        "same currency, null aedEquivalent, non-empty "
        "controller triggers recalculation path", (WidgetTester tester) async {
      final Option usdOption = Option(key: "USD", pairValue: "USD");
      final TextEditingController controller =
      ...
      // Rebuild with same USD but no AED equivalent — triggers recalculation
      // block
      // Lines 258-297 in source: non-AED, null aedEquivalent, controller text
      // non-empty
      await tester.pumpWidget(
        buildCurrencyDropdown(
          options: [Option(key: "AED", pairValue: "AED"), usdOption],
      ...
      // Pump past the 300ms rate debounce + MockInterceptor 500ms delay
      // (see note above on pumpAndSettle and the AED loader).
      await tester.pump(const Duration(milliseconds: 400));
      await tester.pump(const Duration(seconds: 1));
      await tester.pump();
  ...
  group("CurrencyDropdown getCurrencyRates paths", () {
    testWidgets(
        "entering text with non-AED currency "
        "triggers getCurrencyRates via onChanged", (WidgetTester tester) async {
      final Option usdOption = Option(key: "USD", pairValue: "USD");
      final TextEditingController controller = TextEditingController();
      Map<String, dynamic>? emitted;
    ...
    testWidgets(
      ...
      // Pump past the 300ms rate debounce + MockInterceptor delay
      // (see note above on pumpAndSettle and the AED loader).
      await tester.pump(const Duration(milliseconds: 400));
      await tester.pump(const Duration(seconds: 1));
      await tester.pump();
    ...
    testWidgets(
        "didUpdateWidget with null aedEquivalent and "
        "non-AED, non-empty controller covers recalc path",
        (WidgetTester tester) async {
      final Option usdOption = Option(key: "USD", pairValue: "USD");
      final TextEditingController controller =
          TextEditingController(text: "200");
      ...
      // past the 300ms debounce (see note above on pumpAndSettle and the loader)
      await tester.pump(const Duration(milliseconds: 400));
      await tester.pump(const Duration(seconds: 1));
      await tester.pump();

      // Rebuild: same currency, null aedEquivalent → triggers recalculation
      // block
      await tester.pumpWidget(
        buildCurrencyDropdown(
          options: [Option(key: "AED", pairValue: "AED"), usdOption],
