...
GoRouterState createGoRouterState({
  ...
  final effectivePath = fullPath ?? matchedLocation;

  return GoRouterState(
    configuration,
    uri: Uri.parse(effectivePath.isEmpty ? "/" : effectivePath),
    matchedLocation: matchedLocation,
    name: name,
    path: path ?? matchedLocation,
    fullPath: fullPath,
    pathParameters: pathParameters ?? <String, String>{},
    extra: extra,
    pageKey: ValueKey<String>(effectivePath.isEmpty ? "/" : effectivePath),
  );
}
...
void main() {
  ...
  setUpAll(() async {
    ...
    registerFallbackValue(const LayoutState(currentRoute: "/", hideSideMenu: false));
  ...
  group("LayoutViewModel Tests", () {
    group("Initialization", () {
      ...
      testWidgets("init should update state with router location",
        ...
        viewModel.init(context, hideSideMenu: true);
      ...
      testWidgets("init should handle different hideSideMenu values",
        ...
        viewModel.init(context, hideSideMenu: false);
  ...
  group("State Management", () {
    ...
    test("should preserve existing values when not provided in copyWith", () {
      viewModel.emit(const LayoutState(currentRoute: "/test", hideSideMenu: true));
    ...
    test("should emit state changes", () {
      const newState = LayoutState(currentRoute: "/new", hideSideMenu: true);
    ...
    test("should handle multiple state emissions", () {
      const state1 = LayoutState(currentRoute: "/route1", hideSideMenu: false);
      const state2 = LayoutState(currentRoute: "/route2", hideSideMenu: true);
      const state3 = LayoutState(currentRoute: "/route3", hideSideMenu: false);
    ...
    test("should handle state equality", () {
      const state1 = LayoutState(currentRoute: "/test", hideSideMenu: true);
      const state2 = LayoutState(currentRoute: "/test", hideSideMenu: true);
    ...
    test("should handle copyWith with null values", () {
      viewModel
          .emit(const LayoutState(currentRoute: "/original", hideSideMenu: true));
    ...
    test("should handle copyWith with partial updates", () {
      viewModel.emit(const LayoutState(currentRoute: "/start", hideSideMenu: false));
  ...
  group("Extracted Dialog Methods", () {
    ...
    //   try {
    //     Globals.user = User(currentRole: Role(name: 'Temp'));
  ...
  group("goToNextRoute", () {
    ...
    setUp(() {
      ...
      routerState = createGoRouterState(
        configuration: originalRouter.configuration,
      ...
      when(() => mockRouter.state).thenAnswer((_) => routerState);
    ...
    test("calls onAutoSave before navigating", () {
      ...
      when(() => mockRouter.go("/next")).thenReturn(null);
    ...
    test("navigates to route returned by findNextRoute", () {
      ...
      when(() => mockRouter.go("/next-route")).thenReturn(null);

      viewModel.goToNextRoute();

      verify(() => mockRouter.go("/next-route")).called(1);
    });
    ...
    test(
      ...
      () {
        ...
        when(() => mockRouter.go(Routes.queriesAndResponses)).thenReturn(null);

        viewModel.goToNextRoute();

        verify(() => mockRouter.go(Routes.queriesAndResponses)).called(1);
      },
    );
    ...
    test(
      ...
      () {
        ...
        when(() => mockRouter.go("/fallback-route")).thenReturn(null);

        viewModel.goToNextRoute();

        verify(() => mockRouter.go("/fallback-route")).called(1);
      },
    );
    ...
    test("handles null router.state.fullPath safely", () {
      ...
      when(() => mockRouter.go("/safe-route")).thenReturn(null);

      viewModel.goToNextRoute();

      verify(() => mockRouter.go("/safe-route")).called(1);
    });
  });
  ...
  group("goToNextRouteAccess", () {
    ...
    setUp(() {
      ...
      routerState = createGoRouterState(
        configuration: originalRouter.configuration,
        matchedLocation: Routes.creditAssessment,
        fullPath: Routes.creditAssessment,
      );
    ...
    test("skips queries tab when not visible", () {
      routerState = createGoRouterState(
        configuration: originalRouter.configuration,
        matchedLocation: Routes.queriesAndResponses,
        fullPath: Routes.queriesAndResponses,
      );
      ...
      when(() => mockRouter.go(Routes.creditAssessment)).thenReturn(null);

      viewModel.goToNextRouteAccess(MockBuildContext());

      verify(() => mockRouter.go(Routes.creditAssessment)).called(1);
    });
    ...
    test("skips groupSummary when group access is denied", () {
      ...
      when(() => mockRouter.go(Routes.managementComments)).thenReturn(null);

      viewModel.goToNextRouteAccess(MockBuildContext());

      verify(() => mockRouter.go(Routes.managementComments)).called(1);
    });
