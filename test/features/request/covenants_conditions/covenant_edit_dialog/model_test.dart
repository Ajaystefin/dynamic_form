...
Map<String, List<Reference>> _fullRefData() => {
      ...
      ReferenceDataKeys.covenantConditionAction: [
        Reference(id: ServerConstants.covenantCreateActionId, name: "Create"),
...
void main() {
  ...
  setUp(() async {
    ...
    vm = CovenantEditDialogViewModel(null, isNew: false)..repository = mockRepo;
  ...
  tearDown(() async {
    await TestConfig.closeAllBoxes();
  });
  ...
  test("initial state is loading", () {
    final fresh = CovenantEditDialogViewModel(null, isNew: false);
        ...
        vm
          ..covenant = Covenant(
            covenantType:
                ServerConstants.covenantTypeId[CovenantType.financial],
            customerName: "Test",
            action: ServerConstants.covenantCreateActionId,
          )
          ..populateFromExistingCovenant();
        expect(vm.state.loaderStatus, LoadingStatus.loaded);
      });
  ...
  group("init()", () {
    test("init with isNew=true creates new covenant", () async {
      final fresh = CovenantEditDialogViewModel(null, isNew: true)
        ..repository = mockRepo;
      try {
        await fresh.init(null, null, isNew: true);
      } on Object catch (_) {}
      expect(
    ...
    test("init with covenantData populates fields", () async {
      ...
      final fresh = CovenantEditDialogViewModel(covenant, isNew: false)
        ..repository = mockRepo;
      try {
        await fresh.init(null, null, isNew: false, covenantData: covenant);
      } on Object catch (_) {}
    ...
    test("returns empty when selectedAction not in available list", () {
      expect(
        (vm
            ...
            .getSelectedActionItems(forceEmptySelection: false),
        isEmpty,
      );
    });
  ...
  group("date formatting", () {
    test("formatDateForUI pads day and month", () {
      expect(vm.formatDateForDisplay(DateTime(2024, 3, 5)), "05-03-2024");
    });

    test("formatDateForUI two-digit values", () {
      expect(vm.formatDateForDisplay(DateTime(2024, 12, 31)), "31-12-2024");
    });

    test("formatDateForRequest produces yyyy-MM-dd", () {
      expect(vm.formatDateForApiRequest(DateTime(2024, 3, 5)), "2024-03-05");
    });

    test("formatDateForRequest two-digit values", () {
      expect(vm.formatDateForApiRequest(DateTime(2024, 11, 20)), "2024-11-20");
    });
  ...
  group("onCovenantSubTypeSelect", () {
    ...
    test("maps threshold when subtype is in minThresholdSubtypeIds", () {
      if (ServerConstants.minThresholdSubtypeIds.isEmpty) {
        return;
      }
    ...
    test("isNewCovenant=true with null selectedCustomer returns empty", () {
      expect(
        (vm
              ..isNewCovenant = true
              ..selectedCustomer = null)
            .getSelectedCustomerForDropdown(forceShow: false),
        isEmpty,
      );
    });
    ...
    test("forceShow=true returns customer even with isNewCovenant=true", () {
      expect(
        (vm
            ...
            .getSelectedCustomerForDropdown(forceShow: true)
            .length,
        1,
      );
    });
  ...
  group("selection list getters", () {
    test("getSelectedFinancialSubtype: forceEmpty=true returns empty", () {
      expect(
        (vm..selectedFinancialCovenantSubType = Reference(id: 1))
            .getSelectedFinancialSubtype(null, forceEmpty: true),
        isEmpty,
      );
    });
    ...
    test("getSelectedFinancialSubtype: externalSelectedItem wins", () {
      ...
      expect(vm.getSelectedFinancialSubtype(ext, forceEmpty: false).first, ext);
    });
    ...
    test(
      ...
      expect(
        (vm..selectedFinancialCovenantSubType = Reference(id: 7, name: "Int"))
            .getSelectedFinancialSubtype(null, forceEmpty: false)
            .first
            .id,
        7,
      );
    ...
    test("getSelectedFinancialSubtype: both null returns empty", () {
      ...
      expect(vm.getSelectedFinancialSubtype(null, forceEmpty: false), isEmpty);
    });
    ...
    test("getSelectedThreshold: externalSelectedItem wins", () {
      ...
      expect(vm.getSelectedThreshold(ext, forceEmpty: false).first, ext);
    });

    test("getSelectedThreshold: internal returned", () {
      expect(
        (vm..selectedThreshold = Reference(id: 6))
            .getSelectedThreshold(null, forceEmpty: false)
            .first
            .id,
        6,
      );
    ...
    test("getSelectedThreshold: both null returns empty", () {
      ...
      expect(vm.getSelectedThreshold(null, forceEmpty: false), isEmpty);
    });
  ...
  group("initializeDefaultActionIfNeeded", () {
    test("new covenant + no action: sets Create action", () {
      vm
        ...
        ..initializeDefaultActionIfNeeded();
      expect(vm.selectedAction?.id, ServerConstants.covenantCreateActionId);
    });
  ...
  // ════════════════════════════════════════════
  //  Additional public API coverage for model.dart
  // ════════════════════════════════════════════
  group("additional public API coverage", () {
    test("enum/getter helpers return stable values", () {
      vm
        ..selectedCovenantType = Reference(
          id: ServerConstants.covenantTypeId[CovenantType.financial],
          name: "Financial",
        )
        ..selectedCovenantSubType = Reference(id: 1, name: "Subtype")
        ..selectedGeneralCovenantSubType = Reference(id: 1, name: "GeneralSub")
        ..selectedFinancialCovenantSubType = Reference(id: 1, name: "FinSub")
        ..generalField = Reference(
          id: ServerConstants.covenantSpecificId,
          name: "Specific",
        );

      expect(vm.isSpecificSelected(), true);
      expect(vm.selectedCovenantTypeEnum, CovenantType.financial);
      expect(
        vm.selectedSubTypeValueEnum,
        anyOf(isNull, isA<CovenantSubType>()),
      );
      expect(
        vm.selectedSubGeneralTypeValueEnum,
        anyOf(isNull, isA<CovenantSubType>()),
      );
      expect(
        vm.selectedSubFinancialTypeValueEnum,
        anyOf(isNull, isA<CovenantSubType>()),
      );
      expect(vm.isCancellationApp, isA<bool>());
      expect(vm.canEditComments, isA<bool>());
    });

    test("selectedSubTypeItems returns selected general subtype only", () {
      expect(vm.selectedSubTypeItems, isEmpty);
      vm.selectedGeneralCovenantSubType = Reference(id: 44, name: "Selected");
      expect(vm.selectedSubTypeItems.single.id, 44);
    });

    test("main and row threshold enable/require helpers cover custom branches",
        () {
      final customRow = Covenant()
        ..isStandard = false
        ..covenantSubType = 88888;
      final standardRow = Covenant()
        ..isStandard = true
        ..covenantSubType = 88888;
      vm
        ..selectedFinancialDescriptionTypeId =
            ServerConstants.customDescriptionId
        ..selectedFinancialCovenantSubType = Reference(id: 88888, name: "NoMap")
        ..covenant = (Covenant()..covenantSubType = 88888);
      expect(vm.isCustomFinancialDescriptionSelected, true);
      expect(vm.shouldEnableMainThresholdType, true);
      expect(vm.shouldRequireMainThresholdType, true);
      expect(vm.isCustomRowDescriptionSelected(customRow), true);
      expect(vm.shouldEnableRowThresholdType(customRow), true);
      expect(vm.shouldRequireRowThresholdType(customRow), true);
      expect(vm.isCustomRowDescriptionSelected(standardRow), false);
      expect(vm.shouldEnableRowThresholdType(standardRow), true);
      expect(vm.shouldRequireRowThresholdType(standardRow), false);
    });

    test("threshold mapping helper returns mapped and unmapped values", () {
      if (ServerConstants.minThresholdSubtypeIds.isNotEmpty) {
        final ref = vm.getThresholdTypeForCovenantSubtype(
          ServerConstants.minThresholdSubtypeIds.first,
        );
        expect(ref?.id, ServerConstants.thresholdTypeMin);
      }
      if (ServerConstants.maxThresholdSubtypeIds.isNotEmpty) {
        final ref = vm.getThresholdTypeForCovenantSubtype(
          ServerConstants.maxThresholdSubtypeIds.first,
        );
        expect(ref?.id, ServerConstants.thresholdTypeMax);
      }
      expect(vm.getThresholdTypeForCovenantSubtype(null), isNull);
      expect(vm.getThresholdTypeForCovenantSubtype(987654), isNull);
    });

    test("description template and row description helpers", () {
      expect(vm.getDescriptionTemplateForSubtype(null), "");
      expect(vm.getDescriptionTemplateForSubtype(987654), contains("{value}"));
      final row = Covenant()..covenantSubType = 1;
      final description = vm.buildStandardRowDescription(row, "123");
      expect(description, contains("123"));
      expect(description, isNotEmpty);
    });

    test("bracket extraction and financial sanitization helpers", () {
      vm.selectedCovenantType = Reference(
        id: ServerConstants.covenantTypeId[CovenantType.financial],
        name: "Financial",
      );
      expect(vm.extractFirstBracketContent("A [ 123 ] B"), " 123 ");
      expect(vm.extractFirstBracketContent("A [ {value} ] B"), "");
      expect(vm.getBracketRawValue("A [ 123 ]"), "123");
      expect(vm.hasEmptyBracketContent("A [   ]"), true);
      expect(vm.hasEmptyBracketContent("A with no brackets"), false);
      expect(vm.sanitizeAndClampBracketInput("abc12.3.4xyz"), "12.34");
      expect(
        vm.sanitizeAndClampBracketInput("123456789012345678.999"),
        "1234567890123456.99",
      );
      expect(vm.getFirstEditableCharacterIndexInsideBrackets("A [ 99 ]"), 4);
      expect(
        vm.getFirstEditableCharacterIndexInsideBrackets("No brackets"),
        -1,
      );
    });

    test("non-financial bracket sanitization preserves allowed spaces", () {
      vm.selectedCovenantType = Reference(
        id: ServerConstants.covenantTypeId[CovenantType.nonFinancial],
        name: "Non-Financial",
      );
      expect(vm.isNonFinancialBracketInputMode, true);
      expect(
        vm.sanitizeAndClampBracketInput(
          " Ab c!@#123 ",
          maxLength: 5,
      ...
      expect(vm.getFirstEditableCharacterIndexInsideBrackets("A [ abc ]"), 4);
    });

    test("financial caret and threshold parsing helpers", () {
      const rebuilt = "Ratio [ 123.45 ]";
      final caret = vm.calculateFinancialBracketCaretOffset(
        inputText: "Ratio [ 12a3.456 ]",
        rebuiltText: rebuilt,
        currentCaret: "Ratio [ 12a3.456".length,
      );
      expect(caret, inInclusiveRange(8, rebuilt.indexOf("]")));
      expect(
        vm.calculateFinancialBracketCaretOffset(
          inputText: "No bracket",
          rebuiltText: rebuilt,
          currentCaret: 3,
        ),
        rebuilt.length,
      ...
      expect(
        vm.parseThresholdNumberFromDescription("Ratio [ 123.45 ]"),
        123.45,
      );
      expect(vm.parseThresholdNumberFromDescription("Ratio [ . ]"), isNull);
    });

    test("financial description change covers guard and update flows", () {
      vm
        ..covenant = Covenant()
        ..selectedCovenantType = Reference(
          id: ServerConstants.covenantTypeId[CovenantType.financial],
        ...
        ..selectedFinancialCovenantSubType = Reference(id: 1, name: "FinSub")
        ..selectedSubTypeValue = Reference(id: 1, name: "SubValue")
        ..isUpdatingFinancialDescription = true
        ..onFinancialDescriptionChanged("ignored");
      expect(vm.isUpdatingFinancialDescription, true);
      vm.isUpdatingFinancialDescription = false;
      vm.financialDescriptionController.value = const TextEditingValue(
        text: "FinSub Shall not exceed [ 12a3.456 ]",
        selection: TextSelection.collapsed(offset: 31),
      );
      vm.onFinancialDescriptionChanged("FinSub Shall not exceed [ 12a3.456 ]");
      expect(vm.financialDescriptionController.text, contains("123.45"));
      expect(vm.covenant?.threshold, isNull);
    });

    test("description type financial standard updater", () {
      vm
        ..descriptionTypes = [
          Reference(
            id: ServerConstants.standardDescriptionId,
        ...
        ..selectedFinancialDescriptionType = "Standard"
        ..updateFinancialIsStandardFromSelection();
      expect(vm.isFinancialStandard, true);
      vm
        ..selectedFinancialDescriptionType = "Custom"
        ..updateFinancialIsStandardFromSelection();
      expect(vm.isFinancialStandard, false);
    });

    test("simple selection and add/cancel UI toggle helpers", () {
      vm.onSubtypeSelection([Reference(id: 22, name: "SubtypeValue")]);
      expect(vm.selectedSubTypeValue?.id, 22);
      vm.onAddButtonPress();
    ...
    test("custom financial description sanitization and sync helpers", () {
      expect(
        vm.sanitizeCustomFinancialDescription("Ratio [ ab12.3.4xyz ]"),
        "Ratio [ 12.34 ]",
      );
      expect(
        vm.sanitizeCustomFinancialDescription("No bracket text"),
        "No bracket text",
      );
      vm
        ..covenant = Covenant()
        ..syncThresholdFromCustomFinancialDescription("Ratio [ 123 ]");
      expect(vm.covenant?.description, "Ratio [ 123 ]");
      expect(vm.covenant?.threshold, 123);
      final row = Covenant();
      vm.syncThresholdFromCustomFinancialDescription(
        "Row [ 456 ]",
        target: row,
      );
      expect(row.description, "Row [ 456 ]");
      expect(row.threshold, 456);
      final changed = vm.syncThresholdFromCustomFinancialDescriptionAndRefresh(
        "Ratio [ ab789.123 ]",
      );
      expect(changed, true);
      expect(vm.covenant?.description, "Ratio [ 789.12 ]");
      final unchanged =
          vm.syncThresholdFromCustomFinancialDescriptionAndRefresh(
        "Ratio [ 789.12 ]",
      );
      expect(unchanged, false);
    });

    test("filtered subtype helpers cover non-financial and type-based branches",
        () {
      vm.referenceData[ReferenceDataKeys.covenantSubtype] = [
        Reference(
          id: 10,
          name: "NonFinancialSub",
          reference2: ServerConstants.covenantTypeIdNonFinancial.toString(),
        ),
        Reference(
          id: 11,
      ...
      vm
        ..showOnlyNonFinancialSubtypeItems = true
        ..covenant = (Covenant()
          ..covenantType = ServerConstants.covenantTypeIdNonFinancial);
      expect(
          vm.getFilteredFinancialCovenantSubtypes().map((e) => e.id).toList(), [
        10,
        ServerConstants.covenantSubTypeId[CovenantSubType.other],
      ]);
      expect(vm.getFilteredCovenantSubtypesByType().map((e) => e.id).toList(), [
        10,
        ServerConstants.covenantSubTypeId[CovenantSubType.other],
      ]);
    });
