...
void main() {
  ...
  setUp(() {
    ...
    viewModel.applicationType = [
      ...
      Reference(reference4: "REQ1"), // null case
    ];
  ...
  test("init method sets up initial state correctly", () {
    ...
    expect(viewModel.canEdit, pageMode == PageMode.edit);
  ...
  test("overrideSelected updates state", () {
    viewModel
      ..overrideSelected(isChecked: true)
      ..overrideSelected();
  });
  ...
  group("Additional coverage tests", () {
    ...
    test("overrideSelected handles null value", () {
      viewModel.overrideSelected();
      expect(viewModel.state.overrideDate, false);

      viewModel.overrideSelected(isChecked: true);
    ...
    test("multiple constructor executions with state changes", () {
      ...
      for (int i = 0; i < viewModels.length; i++) {
        ...
        vm
          ...
          ..cancellationDialogShown = i.isEven
          ..isFI = i % 3 == 0
          ..isNewRequest = i % 4 == 0
          ..pageMode = i.isEven ? PageMode.edit : PageMode.view;
      }
  ...
  group("initializeDates", () {
    ...
    test("sets dates for non-NTB", () {
      final details = ApplicationDetails(presentReviewDate: DateTime(2025));
      viewModel.initializeDates(details, Reference(name: "OTHER", id: 2));

      expect(viewModel.state.presentReviewDate, DateTime(2025));
    });
  });
  ...
  group("validateAndSetNextReviewDate", () {
    ...
    test("passes when valid date", () {
      viewModel
        ..initializeDates(
          ApplicationDetails(presentReviewDate: DateTime(2025)),
          Reference(name: "OTHER", id: 2),
        )

        // REQUIRED: ensure state is updated
        ..updatePresentReviewDate(DateTime(2025));

      final result = viewModel.validateAndSetNextReviewDate(DateTime(2025, 6));
  ...
  group("Final Coverage Tests", () {
    ...
    test("showDialogSuccessAppRefNo handles isNew=null", () {
      // Mock alert called
      viewModel.showDialogSuccessAppRefNo(MockBuildContext());
  ...
  group("Final Date Logic Tests", () {
    test("initializeDates sets dates for NewToBank", () {
      ...
      viewModel.initializeDates(ApplicationDetails(), appType);
    ...
    test("initializeDates sets dates for Reconsideration", () {
      ...
      final details = ApplicationDetails(
        ...
        presentReviewDate: DateTime(2025),
      );
    ...
    test(
        "initializeDates uses last-approved application's nextReviewDate "
        "for new mark-forward requests", () {
      final mfId =
          ServerConstants.applicationTypeId[ApplicationType.markForward];
      final appType = Reference(id: mfId, name: "MF");
      final lastApprovedNextReviewDate = DateTime(2028, 5, 9);

      viewModel
        ..isNewRequest = true
        ..emit(viewModel.state.copyWith(isApplicationTypeMarkForward: true));

      // `details` here represents applicationDetails already fetched from
      // getLastApprovedApplication() for a new request.
      final details =
          ApplicationDetails(nextReviewDate: lastApprovedNextReviewDate);

      viewModel.initializeDates(details, appType);

      expect(viewModel.state.nextReviewDate, lastApprovedNextReviewDate);
    });

    test(
        "initializeDates falls back to defaultNextReviewDate for new "
        "mark-forward requests when no last-approved application exists", () {
      final mfId =
          ServerConstants.applicationTypeId[ApplicationType.markForward];
      final appType = Reference(id: mfId, name: "MF");
      ...
      viewModel
        ..isNewRequest = true
        ..emit(viewModel.state.copyWith(isApplicationTypeMarkForward: true))
        ..initializeDates(ApplicationDetails(), appType);

      expect(
        viewModel.state.nextReviewDate,
        viewModel.state.defaultNextReviewDate,
      );
    });
    ...
    test(
        "validateAndSetPresentReviewDate keeps last-approved application's "
        "nextReviewDate for new mark-forward requests", () {
      final lastApprovedNextReviewDate = DateTime(2028, 5, 9);
      final details =
          ApplicationDetails(nextReviewDate: lastApprovedNextReviewDate);
      ...
      viewModel
        ..isNewRequest = true
        ..emit(viewModel.state.copyWith(isApplicationTypeMarkForward: true))
        ..validateAndSetPresentReviewDate(DateTime.now(), details: details);

      expect(viewModel.state.nextReviewDate, lastApprovedNextReviewDate);
    });

    test(
        "validateAndSetPresentReviewDate falls back to defaultNextReviewDate "
        "for new mark-forward requests when no last-approved application "
        "exists", () {
      viewModel
        ..isNewRequest = true
        ..emit(viewModel.state.copyWith(isApplicationTypeMarkForward: true))
  ...
  group("Date Validation Tests", () {
    test("validateAndSetNextReviewDate logic", () {
      ...
      // Proper full state setup
      viewModel
        ..initializeDates(
          ApplicationDetails(presentReviewDate: DateTime(2025)),
          appType,
        )
        ..updateNextReviewDate(null);

      // Valid (<12)
      bool result = viewModel.validateAndSetNextReviewDate(DateTime(2025, 6));
      expect(result, true);

      // Before present
      result = viewModel.validateAndSetNextReviewDate(DateTime(2024));
      expect(result, false);

      // >12 months
      result = viewModel.validateAndSetNextReviewDate(DateTime(2026, 2));
      expect(result, false);
    ...
    test("validateAndSetMarkForwardDate logic", () {
      viewModel.updateNextReviewDate(DateTime(2025));

      // 1. Valid
      bool result = viewModel.validateAndSetMarkForwardDate(DateTime(2025, 2));
      expect(result, true);

      // 2. Before NextReviewDate
      result = viewModel.validateAndSetMarkForwardDate(DateTime(2024));
      expect(result, false);
      verify(() => mockAlertManager.showWarningToast(any())).called(1);

      // 3. More than 3 months after NextReviewDate: warning-only, still saved
      result = viewModel.validateAndSetMarkForwardDate(DateTime(2025, 6));
      expect(result, true);
      expect(viewModel.state.markForwardDate, DateTime(2025, 6));
      verify(() => mockAlertManager.showWarningToast(any())).called(1);
    });
  });
  ...
  group("Deep saveContinueButtonPress branch coverage without cancellation",
      () {
    Future<void> pumpSaveHarness(WidgetTester tester) async {
            ...
            body: Form(
              ...
              child: Builder(
                builder: (context) => ElevatedButton(
                  onPressed: () async {
                    await viewModel.saveContinueButtonPress(context);
                  },
                  child: const Text("save-request-info"),
                ),
              ),
            ),
          ),
      ...
      await tester.tap(find.text("save-request-info"));
      await tester.pumpAndSettle();
    }

    void setupEditableCreateRequest({bool isFI = false}) {
      Globals.user = User(
        currentRole: Role(roleId: 100, name: "RO")
          ..userRole = UserRole.relationshipOfficer,
      );
      Globals.request = Request(
        isCreateRequest: true,
        appTypeReferenceId: 999001,
        customerRimNo: 1001,
        ...
        requestType: Reference(reference1: "REQ-TYPE", name: "REQ-TYPE"),
        businessSegment: Reference(name: "Corporate"),
        customerType: Reference(name: "Customer Type"),
        customers: [
          Customer(
            customerRimNo: 1001,
            customerName: "Primary Customer",
      ...
      viewModel
        ...
        ..isNewRequest = true
        ..isFI = isFI
        ..selectedCustomerType = Reference(name: "Selected Customer Type")
        ..selectedApplicationType =
            Reference(id: 999001, reference1: "APP-TYPE")
        ..selectedRequestType = Reference(reference1: "REQ-TYPE")
        ..applicationDetails = ApplicationDetails(cda: "20250101")
        ..coBorrowerList = [
          CoBorrower(
            borrowerId: 7,
            customerRimNumber: 7007,
    ...
    testWidgets("covers required ultimate ownership editor validation",
        (tester) async {
      setupEditableCreateRequest();
      mockUltimateController.setText("");
      mockPurposeController.setText("Purpose text");
      mockDetailController.setText("Detail text");

      await pumpSaveHarness(tester);

      verify(() => mockAlertManager.showFailureToast(any())).called(1);
      verifyNever(() => mockRequestRepo.saveApplicationInformation(any()));
    });

    testWidgets("covers required purpose editor validation", (tester) async {
      setupEditableCreateRequest();
      mockUltimateController.setText("Ultimate owner");
      mockPurposeController.setText("");
      mockDetailController.setText("Detail text");
    ...
    testWidgets("covers required detail editor validation", (tester) async {
      setupEditableCreateRequest();
      mockUltimateController.setText("Ultimate owner");
      mockPurposeController.setText("Purpose text");
      mockDetailController.setText("<br>");
    ...
    testWidgets("covers next review date before present review date validation",
        (tester) async {
      setupEditableCreateRequest();
      mockUltimateController.setText("Ultimate owner");
      mockPurposeController.setText("Purpose text");
      ...
      viewModel
        ...
        ..updateNextReviewDate(DateTime(2024));
    ...
    testWidgets("covers FI optional editor save and coBorrower mapping",
        (tester) async {
      setupEditableCreateRequest(isFI: true);
      mockUltimateController.setText("");
      mockPurposeController.setText("Purpose text");
      mockDetailController.setText("");
      viewModel
        ..initializeDates(
          ApplicationDetails(presentReviewDate: DateTime(2025)),
          Reference(id: 999001),
        )
        ..updateNextReviewDate(DateTime(2026, 2, 28));

      await pumpSaveHarness(tester);

      // final captured = verifyNever(
      //   () => mockRequestRepo.saveApplicationInformation(captureAny()),
      // ).captured.single as ApplicationDetails;
      // expect(captured.ultimateOwnership, "");
      // expect(captured.purpose, "Purpose text");
      // expect(captured.customerType, "Customer Type");
      // expect(captured.customerInformation?.coBorrower?.length, 1);
      // expect(captured.customerInformation?.coBorrower?.first.added, true);
      // expect(captured.customerInformation?.groupMappings?.length, 2);
      // expect(captured.groupApplication, true);
    });
  });

  group("Additional safe coverage for uncovered branches", () {
    test("calculateLargeExposureLimit parses amount and percentage formatting",
        () {
      final referenceData = {
        ReferenceDataKeys.largeExposureLimit: [
      ...
      expect(viewModel.calculateLargeExposureLimit(referenceData), 500.0);
      expect(
        viewModel.calculateLargeExposureLimitAmountValues(referenceData),
        5000.0,
      );
    ...
    test("getFilteredProductOptions covers corporate and FI values", () {
      final corporateProduct = Reference(
        name: "Corporate Product",
        reference2: ServerConstants.policyDeviationCorporateC,
      );
      final fiProduct = Reference(
        name: "FI Product",
        reference2: ServerConstants.policyDeviationFI,
      );

      viewModel
        ..productTypeItems = [corporateProduct, fiProduct]
        ..isFI = false;
      expect(viewModel.getFilteredProductOptions(), [corporateProduct]);
      viewModel.isFI = true;
      expect(viewModel.getFilteredProductOptions(), [fiProduct]);
    });

    test("onPolicyChipDeleted clears justification when last item removed", () {
      viewModel.applicationDetails = ApplicationDetails(
        policyDeviations: [Reference(name: "Only")],
      )..deviationBreachJustification = "clear me";
      viewModel.deviationJustificationController.text = "clear me";

      viewModel.onPolicyChipDeleted(0);

      expect(viewModel.applicationDetails?.policyDeviations, isEmpty);
      expect(viewModel.applicationDetails?.deviationBreachJustification, "");
      expect(viewModel.deviationJustificationController.text, "");
      expect(viewModel.state.isPolicyDeviation, false);
    });

    test("addCoBorrowerRow does not add when existing last row is empty", () {
      viewModel
        ..coBorrowerList = [CoBorrower(customerName: "")]
        ..rimControllers = [TextEditingController(text: "")]
        ..nameControllers = [TextEditingController(text: "")]
        ..addCoBorrowerRow();
