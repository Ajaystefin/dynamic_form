  ...
  group("onSaveButtonPressed()", () {
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        expect(capturedSecurity, isNotNull);
        expect(capturedCovenant, isNotNull);
        expect(capturedCondition, isNotNull);

        expect(capturedSecurity!.length, 1);
        expect(capturedCovenant!.length, 1);
        expect(capturedCondition!.length, 1);
  ...
  group("updateTableStateChanges()", () {
    setUp(() {
      viewModel.securityDeferral = SecurityPerfection(
        securityDeferralList: [
          SecurityDeferral(isChecked: false),
        ],
        covenant: [
          SecurityCovenantCondition(isChecked: false),
        ],
        condition: [
          SecurityCovenantCondition(isChecked: false),
        ],
    ...
    test("updates security item when from == s", () {
      ...
      viewModel.updateTableStateChanges("s", true, 0);
    ...
    test("updates covenant item when from == c", () {
      ...
      viewModel.updateTableStateChanges("c", true, 0);
    ...
    test("updates condition item when from == cd", () {
      ...
      viewModel.updateTableStateChanges("cd", true, 0);
