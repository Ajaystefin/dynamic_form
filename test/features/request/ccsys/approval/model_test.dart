...
class FakeStorage implements StorageInterface {
  ...
  @override
  Future<void> put(String box, String key, value) async {
...
void main() {
  ...
  group("_normalizeRoleToCode via buildRecommendWcasRolesForCurrentUser", () {
    ...
    test("returns empty when currentRole null and availableRoles empty",
        () async {
      Globals.user =
          User(id: "X", name: "Y", currentRole: null, availableRoles: []);
      expect(
  ...
  group("loadReturnUsers", () {
    ...
    test(
      ...
      expect(vm.userListReturn.length, 4);
      expect(vm.userListReturn.first.isHeader, isTrue);
      expect(vm.userListReturn.last.value, "user123");
    });
  ...
  group("getUserRoleNames", () {
    ...
    test("handles role with null bpmRole gracefully", () {
      ...
      final names =
          CcsysApprovalViewModel().getUserRoleNames([Role(bpmRole: null)]);
      expect(names, ["null"]);
    });
  });
      ...
      when(() => ccsysRepo.getUsersByRoles(any())).thenAnswer(
        (_) async => [
          Role(
            bpmRole: "Officer",
            code: "RO",
            users: [User(id: null, name: "NoId User")],
          ),
        ],
      );
