...
import "../previous_credit_approval/model_test.dart";
...
void main() {
  ...
  group("getUserListByGroup", () {
    test("should return data if data is present", () async {
      ...
      when(
        () => mockApprovalRepo.getUsersByRoles(any()),
      ).thenAnswer((_) async => mockResponse);
      final groupList =
          await viewModel.getUserListByGroup(FOLTypeAction.returnFromDocCCU);
    ...
    test("should match with the type", () async {
      ...
      final mockResponse = User(
        id: "789",
        name: "User3",
        currentRole: Role(name: "DM", roleId: 2, bpmRole: "DM-WCAS"),
      );
      when(
        () => mockApprovalRepo.getUsersByRoles(any()),
      ).thenAnswer((_) async => [mockResponse]);
    ...
    test("should return empty data if reference data is empty", () async {
      Globals.user = User(id: "u1", currentRole: Role(roleId: 10, code: "DM"));
      Globals.superUserRoles = [
        {
          "RM": "RM-WCAS",
          "RO": "RO-WCAS",
