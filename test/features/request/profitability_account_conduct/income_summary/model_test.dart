...
class TestableIncomeSummaryViewModel extends IncomeSummaryViewModel {
  ...
  // view model without exposing a public getter solely for the setter contract.
  // ignore: avoid_setters_without_getters
  set setMockRepository(MockProfitabilityRepository mock) {
    repository = mock;
  }

  // instance with a mock implementation.
  // ignore: avoid_setters_without_getters
  set setMockCommonRepository(MockCommonRepository mock) {
    CommonRepository.overrideInstance = mock;
  }

  // with a test double for toast verification.
  // ignore: avoid_setters_without_getters
  set setAlertManager(AlertManager alert) {
  ...
  group("IncomeSummaryViewModel lifecycle & state", () {
    ...
    group("saveIncomeSummaryData()", () {
      ...
      testWidgets("loaderStatus transitions: loading -> loaded on success",
        ...
        // "loading".
        final future = viewModel.saveIncomeSummaryData(
          FakeBuildContext(),
          navigateNext: false,
        );
    ...
    group("IncomeSummaryState.copyWith", () {
      test("constructor sets loaderStatus", () {
        const s = IncomeSummaryState(loaderStatus: LoadingStatus.loading);
        expect(s.loaderStatus, LoadingStatus.loading);
      });

      test("copyWith keeps existing when null", () {
        const original = IncomeSummaryState(loaderStatus: LoadingStatus.loaded);
