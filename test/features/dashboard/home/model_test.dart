...
void main() {
    ...
    test("getTableData success populates draftTableData and filteredRequests",
      ...
      when(
        () => mockRepo.getWorkList(
          ...
          isBarGraph: false,
          isInitialLoad: any(named: "isInitialLoad"),
          isAgeingSelected: any(named: "isAgeingSelected"),
        ),
  ...
  group("getUsersForAssigne", () {
    ...
    test(
      "fetches assignee users when BPM roles exist and app ref is provided",
      () async {
        viewModel
          ..summaryData = Summary(
            pendingWithMe: {
        ...
        final mockUsers = [
          User(id: "U10", name: "Assignee 1"),
        ];

        when(() => mockRepo.getUsersForAssigne(any(), any()))
            .thenAnswer((_) async => mockUsers);

        await viewModel.getUsersForAssigne("APP001");

        expect(viewModel.showAssignUser, true);
        expect(viewModel.userForAssigne, mockUsers);
        verify(
          () => mockRepo.getUsersForAssigne(["RM", "CA"], "APP001"),
        ).called(1);
      },
    );
  });
